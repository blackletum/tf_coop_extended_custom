...
LINK_ENTITY_TO_CLASS( npc_xen_grunt, CNPC_AlienGrunt );
...
void CNPC_AlienGrunt::Spawn()
{
	...
	ClearEffects();
	m_iMaxHealth = m_iHealth = sk_agrunt_health.GetFloat();
	...
	case AGRUNT_AE_LEFT_PUNCH:
		{
			...
			CBaseEntity *pHurt = CheckTraceHullAttack( AGRUNT_MELEE_DIST, vecMins, vecMaxs, sk_agrunt_dmg_punch.GetFloat(), iDmgType );
