...
void CNPC_Gonome::Spawn()
{
	...
	m_iMaxHealth = m_iHealth = sk_gonome_health.GetFloat();
...
void CNPC_Gonome::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
		...
		case GONOME_AE_BITE:
		{
			...
			// SOUND HERE!
			CPASAttenuationFilter filter( this );
			CBaseEntity *pHurt = CheckTraceHullAttack( 70, Vector(-16,-16,-16), Vector(16,16,16), sk_gonome_dmg_bite.GetFloat(), iDmgType );
			if ( pHurt )
			{
				Vector forward, up;
