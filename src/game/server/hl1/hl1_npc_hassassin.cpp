...
LINK_ENTITY_TO_CLASS( npc_human_assassin, CNPC_HAssassin );
...
void CNPC_HAssassin::Spawn()
{
    ...
    ClearEffects();
	m_iMaxHealth = m_iHealth = sk_hassassin_health.GetFloat();
...
void CNPC_HAssassin::RunAI( void )
{
	...
	if ( TFGameRules()->GetSkillLevel() < SKILL_HARD || GetEnemy() == NULL || m_lifeState == LIFE_DEAD || GetActivity() == ACT_RUN || GetActivity() == ACT_WALK || !( GetFlags() & FL_ONGROUND ) )
