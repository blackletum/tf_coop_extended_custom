...
void CNPC_HL1Barnacle::Spawn()
{
	...
	m_iMaxHealth = m_iHealth = sk_barnaclehl1_health.GetFloat();
...
// barnacle has prey.
	...
	// still pulling prey.
		...
		// prey has just been lifted into position ( if the victim origin + eye height + 8 is higher than the bottom of the barnacle, it is assumed that the head is within barnacle's body )
			...
			if( trace.fraction != 1.0 )
			{
				...
				if( pEnemy->MyCombatCharacterPointer() )
				{
					...
					if ( pEnemy->IsPlayer() )
					{
						...
						pEnemy->RemoveFlag( FL_ATCONTROLS );
					}
			...
			GetEnemy()->SetAbsOrigin( vecNewEnemyOrigin );
		}
		else
		{
	// prey is lifted fully into feeding position and is dangling there.
			...
			if ( m_flKillVictimTime != -1 && gpGlobals->curtime > m_flKillVictimTime )
			{
				...
				if ( pVictim )
				{
					...
					int iDmgType = DMG_SLASH | DMG_ALWAYSGIB | DMG_CRUSH;
					...
					pVictim->TakeDamage( CTakeDamageInfo( this, this, pVictim->m_iHealth, iDmgType ) );
					m_cGibs = 3;
				}
		...
		if ( pTouchEnt != NULL )
		{
			...
			if ( pBCC && pBCC->DispatchInteraction( g_interactionHL1BarnacleVictimGrab, &vecGrabPos, this ) )
			{
				...
				if ( pTouchEnt->IsPlayer() )
					pTouchEnt->AddFlag( FL_ATCONTROLS );
...
void CNPC_HL1Barnacle::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if ( GetEnemy() != NULL )
	{
		...
		if ( pVictim )
		{
			...
			if ( pVictim->IsPlayer() )
			{
				pVictim->SetMoveType( MOVETYPE_WALK );
				pVictim->RemoveFlag( FL_ATCONTROLS );
			}
