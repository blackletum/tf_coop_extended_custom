...
LINK_ENTITY_TO_CLASS( npc_human_scientist, CNPC_Scientist );
LINK_ENTITY_TO_CLASS( npc_human_scientist_kleiner, CNPC_Scientist );
LINK_ENTITY_TO_CLASS( npc_human_scientist_eli, CNPC_Scientist );
LINK_ENTITY_TO_CLASS( npc_human_scientist_female, CNPC_Scientist );
...
void CNPC_Scientist::Spawn( void )
{
	...
	ClearEffects();
	m_iMaxHealth = m_iHealth = sk_scientist_health.GetFloat();
...
int CNPC_Scientist::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	if ( InSameTeam( inputInfo.GetAttacker() ) && !sv_hl1_ff.GetBool() )
	{
		return 0;
	}
	else
