...
void CNPC_HL1Bullsquid::Spawn()
{
	...
	m_iMaxHealth = m_iHealth = sk_bullsquidhl1_health.GetFloat();
...
void CNPC_HL1Bullsquid::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
		...
		case BSQUID_AE_BITE:
		{
			...
			// SOUND HERE!
			CBaseEntity *pHurt = CheckTraceHullAttack( 70, Vector(-16,-16,-16), Vector(16,16,16), sk_bullsquidhl1_dmg_bite.GetFloat(), iDmgType );
			if ( pHurt )
			{
				Vector forward, up;
		...
		case BSQUID_AE_TAILWHIP:
		{
			CalcIsAttackCritical();
			CalcIsAttackMiniCritical();
			int iDmgType = DMG_SLASH | DMG_ALWAYSGIB;
			...
			CBaseEntity *pHurt = CheckTraceHullAttack( 70, Vector(-16,-16,-16), Vector(16,16,16), sk_bullsquidhl1_dmg_whip.GetFloat(), iDmgType );
			if ( pHurt ) 
			{
				Vector right, up;
