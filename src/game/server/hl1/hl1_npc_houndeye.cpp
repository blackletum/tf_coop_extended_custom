...
void CNPC_HL1Houndeye::Spawn()
{
	...
	ClearEffects();
	m_iMaxHealth = m_iHealth = sk_houndeye_health.GetFloat();
...
int CNPC_HL1Houndeye::GetAttackRadius()
{
	switch (sv_difficulty.GetInt())
	{
	case 2:
		return 768;
	case 3:
...
void CNPC_HL1Houndeye::SonicAttack( void )
{
	...
	while ((pEntity = gEntList.FindEntityInSphere( pEntity, GetAbsOrigin(), GetAttackRadius() )) != NULL)
	{
		if ( pEntity->m_takedamage  != DAMAGE_NO )
		{
			...
			{// houndeyes don't hurt other houndeyes with their attack
				...
				flAdjustedDamage -= ( flDist / GetAttackRadius() ) * flAdjustedDamage;
				...
				if (flAdjustedDamage > 0 )
				{
					...
					int iDmgType = DMG_SONIC | DMG_ALWAYSGIB;
					...
					CTakeDamageInfo info( this, this, flAdjustedDamage, iDmgType );
					CalculateExplosiveDamageForce( &info, (pEntity->GetAbsOrigin() - GetAbsOrigin()), pEntity->GetAbsOrigin() );

					pEntity->TakeDamage( info );

					if ( (pEntity->GetAbsOrigin() - GetAbsOrigin()).Length2D() <= GetAttackRadius() )
					{
