...
LINK_ENTITY_TO_CLASS( npc_xontroller, CNPC_Controller );
...
void CNPC_Controller::Spawn()
{
	...
	m_iMaxHealth = m_iHealth = sk_controller_health.GetFloat();
...
void CNPC_ControllerHeadBall::HuntThink( void  )
{
	...
	if ((GetEnemy()->WorldSpaceCenter() - GetAbsOrigin()).Length() < 64)
	{
		...
		if (pEntity != NULL && pEntity->m_takedamage == DAMAGE_YES)
		{
			...
			CTakeDamageInfo info( this, this, sk_controller_dmgball.GetFloat(), iDmgType );
