...
LINK_ENTITY_TO_CLASS( npc_osprey, CNPC_Osprey );
...
void CNPC_Osprey::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
	...
	if (ptr->hitgroup == 3 )
	{
		...
		if( m_flRightHealth <= 0 )
		{
			Assert( !m_hRightSmoke );

			if ( (m_hRightSmoke = SmokeTrail::CreateSmokeTrail()) == true )
			{
	...
	if (ptr->hitgroup == 2 )
	{
		...
		if( m_flLeftHealth <= 0 )
		{
			...
			if ( (m_hLeftSmoke = SmokeTrail::CreateSmokeTrail()) == true )
			{
...
void CHL1BaseHelicopter::FlyPathCorners( void )
{
	...
	if ( !GetGoalEnt() && m_target != NULL_STRING )// this monster has a target
	{
