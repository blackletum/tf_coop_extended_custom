...
void CNPC_HGrunt::Spawn()
{
	...
	ClearEffects();
	m_iMaxHealth = m_iHealth = sk_hgrunt_health.GetFloat();
	...
	case HGRUNT_AE_KICK:
	{
		...
		if (pHurt)
		{
			...
			if (pHurt->entindex() > 0)
			{
				...
				CTakeDamageInfo info(this, this, sk_hgrunt_kick.GetFloat(), iDmgType);
...
int CNPC_HGrunt::TranslateSchedule(int scheduleType)
{
	...
	case SCHED_TAKE_COVER_FROM_ENEMY:
	{
		if (m_pSquad)
		{
			if ( TFGameRules()->GetSkillLevel() <= SKILL_HARD && HasCondition( COND_CAN_RANGE_ATTACK2 ) && OccupyStrategySlotRange( SQUAD_SLOT_GRENADE1, SQUAD_SLOT_GRENADE2 ) )
			{
				if (FOkToSpeak())
				{
					SENTENCEG_PlayRndSz(edict(), "HG_THROW", HGRUNT_SENTENCE_VOLUME, GRUNT_SNDLVL, 0, m_voicePitch);
