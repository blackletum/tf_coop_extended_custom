...
void CNPC_Nihilanth::Spawn( void )
{
	...
	m_iMaxHealth = m_iHealth = sk_nihilanth_health.GetFloat();
...
void CNihilanthHVR::ZapThink( void  )
{
	...
	if (( GetEnemy()->WorldSpaceCenter() - GetAbsOrigin()).Length() < 256)
	{
		...
		if (pEntity != NULL && pEntity->m_takedamage )
		{
			...
			CTakeDamageInfo info( this, this, sk_nihilanth_zap.GetFloat(), iDmgType );
...
void CNihilanthHVR::ZapTouch( CBaseEntity *pOther )
{
	...
	RadiusDamage( CTakeDamageInfo( this, this, 50, iDmgType ), GetAbsOrigin(), 125,  CLASS_NONE, NULL );
