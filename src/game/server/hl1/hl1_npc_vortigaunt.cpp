...
LINK_ENTITY_TO_CLASS( npc_alien_slave, CNPC_HL1Vortigaunt );
LINK_ENTITY_TO_CLASS( npc_xort, CNPC_HL1Vortigaunt );
...
void CNPC_HL1Vortigaunt::Spawn()
{
	...
	ClearEffects();
    m_iMaxHealth = m_iHealth = sk_islave_health.GetFloat();
...
void CNPC_HL1Vortigaunt::HandleAnimEvent( animevent_t *pEvent )
{
	...
	switch( pEvent->event )
	{
		case ISLAVE_AE_CLAW:
		{
			...
			// SOUND HERE!
			CBaseEntity *pHurt = CheckTraceHullAttack( 40, Vector(-10,-10,-10), Vector(10,10,10), sk_islave_dmg_claw.GetFloat(), iDmgType );
			CPASAttenuationFilter filter( this );
			if ( pHurt )
			{
		...
		case ISLAVE_AE_CLAWRAKE:
		{
			...
			CBaseEntity *pHurt = CheckTraceHullAttack( 40, Vector(-10,-10,-10), Vector(10,10,10), sk_islave_dmg_clawrake.GetFloat(), iDmgType );
...
void CNPC_HL1Vortigaunt::ArmBeam( int side )
{
	...
	if( tr.m_pEnt && tr.m_pEnt->m_takedamage && !tr.m_pEnt->IsNPC() )
	{
		...
		CTakeDamageInfo info( this, this, 10, iDmgType );
...
void CNPC_HL1Vortigaunt::ZapBeam( int side )
{
	...
	if ( pEntity != NULL && m_takedamage )
	{
		...
		CTakeDamageInfo info( this, this, sk_islave_dmg_zap.GetFloat(), iDmgType );
		CalculateMeleeDamageForce( &info, vecAim, tr.endpos );
