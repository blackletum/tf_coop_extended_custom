...
public:
	...
	bool				IsInSquadName( string_t name )  { return m_SquadName == name; }
...
public:
	...
	void	SetLocalizeName( const char *pszName ) { Q_snprintf( m_pszLocalizeName.GetForModify(), MAX_PLAYER_NAME_LENGTH, pszName ); }
	...
	void	Concussion( CTFPlayer *pAttacker, float flDistance );
	void	AttachSapper( CTFPlayer *pAttacker );
	virtual void	OnAttachSapper( void ) {};
	...
	void				DropDeathCallingCard( CTFPlayer *pKiller, CBaseEntity *pVictim );
	void				DropRune( void );
	...
	void				AddTempCritBonus( float flDuration = PERMANENT_ATTRIBUTE );
	...
	virtual void		UpdateLastKnownArea( void );						// invoke this to update our last known nav area (since there is no think method chained to me)
	...
	void				AddCustomAttribute( char const *pAttribute, float flValue, float flDuration = PERMANENT_CONDITION );
...
protected:
	...
	CNetworkVar( bool, m_bGoldDeath );
	CNetworkVar( bool, m_bIceDeath );
	CNetworkVar( bool, m_bAshDeath );
	CNetworkVar( int, m_iDamageCustomDeath );
...
private:
	...
	CNetworkString( m_pszLocalizeName, MAX_PLAYER_NAME_LENGTH );
