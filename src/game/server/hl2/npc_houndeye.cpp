...
extern ConVar sk_houndeye_health;
extern ConVar sk_houndeye_dmg_blast;
...
void CNPC_Houndeye::Spawn()
{
	if ( TFGameRules() && TFGameRules()->IsHL2Beta() )
	{
		...
		SetModel("models/houndeye_hd.mdl"); //TODO: Makea  unique model for this
...
void CNPC_Houndeye::StartTask( const Task_t *pTask )
{
	...
	case TASK_HOUND_GET_PATH_TO_CIRCLE:
	{
		...
		else 
		{
			...
			if (GetNavigator()->SetRadialGoal(vTargetPos, (Vector)random->RandomInt(50,500), 90, 175, m_bLoopClockwise,true))
			{
				TaskComplete();
				return;
			}
