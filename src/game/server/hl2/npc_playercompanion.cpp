...
void CNPC_PlayerCompanion::Spawn()
{
	...
	{
		DevWarning("NPC %s using alternate collision! -- DISABLED\n", STRING(GetEntityName()));
...
#ifdef TF_CLASSIC
			|| (info.GetDamageCustom() & TF_DMG_CUSTOM_BURNING) || (info.GetDamageCustom() & TF_DMG_CUSTOM_BURNING_ARROW) || (info.GetDamageCustom() & TF_DMG_CUSTOM_DRAGONS_FURY_IGNITE) || (info.GetDamageCustom() & TF_DMG_CUSTOM_DRAGONS_FURY_BONUS_BURNING)
#endif
			)
		{
...
bool CNPC_PlayerCompanion::OnObstructionPreSteer(AILocalMoveGoal_t *pMoveGoal, float distClear, AIMoveResult_t *pResult)
{
	if ( pMoveGoal->directTrace.flTotalDist - pMoveGoal->directTrace.flDistObstructed < GetHullWidth() * 1.5 )
	{
		...
		if (pBlocker && InSameTeam( pBlocker ) && !pBlocker->IsMoving() && !pBlocker->IsInAScript() &&
