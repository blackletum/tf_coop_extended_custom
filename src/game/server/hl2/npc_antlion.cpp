...
// base antlion stuff
ConVar	sk_antlion_health( "sk_antlion_health", "30", FCVAR_REPLICATED );
ConVar	sk_antlion_swipe_damage( "sk_antlion_swipe_damage", "5", FCVAR_REPLICATED );
ConVar	sk_antlion_jump_damage( "sk_antlion_jump_damage", "5", FCVAR_REPLICATED );
ConVar  sk_antlion_air_attack_dmg( "sk_antlion_air_attack_dmg", "0", FCVAR_REPLICATED );
...
ConVar  sk_antlion_worker_burst_damage( "sk_antlion_worker_burst_damage", "50", FCVAR_REPLICATED, "How much damage is inflicted by an antlion worker's death explosion." );
ConVar	sk_antlion_worker_health( "sk_antlion_worker_health", "0", FCVAR_REPLICATED, "Hitpoints of an antlion worker. If 0, will use base antlion hitpoints."   );
ConVar  sk_antlion_worker_spit_speed( "sk_antlion_worker_spit_speed", "0", FCVAR_REPLICATED, "Speed at which an antlion spit grenade travels." );

// This must agree with the AntlionWorkerBurstRadius() function!
ConVar  sk_antlion_worker_burst_radius( "sk_antlion_worker_burst_radius", "160", FCVAR_REPLICATED, "Effect radius of an antlion worker's death explosion."  );

#endif

ConVar 	g_test_new_antlion_jump( "g_test_new_antlion_jump", "1", FCVAR_CHEAT );
ConVar	antlion_easycrush( "antlion_easycrush", "1", FCVAR_REPLICATED );
ConVar	g_antlion_cascade_push( "g_antlion_cascade_push", "1", FCVAR_CHEAT );
 
ConVar	g_debug_antlion_worker( "g_debug_antlion_worker", "0", FCVAR_CHEAT );
...
void CNPC_Antlion::Spawn( void )
{
	...
	m_iMaxHealth = m_iHealth = ( IsWorker() ) ? sk_antlion_worker_health.GetFloat() : sk_antlion_health.GetFloat();
	...
	// MapBase: Implement dynamic interactions here since we can't recompile the model
	if ( GetModelPtr() )
	{
...
int CNPC_Antlion::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if ( !IsFlipped() && ( info.GetAttacker() && info.GetAttacker()->IsPlayer() ) && ( newInfo.GetDamageType() & DMG_CLUB|DMG_SLASH ) )
		flDamage /= 1.5;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_Antlion::IsFlipped( void ) 
{
...
int CNPC_Antlion::MeleeAttack1Conditions( float flDot, float flDist )
{
	...
	if ( tr.m_pEnt != GetEnemy() && tr.fraction < 1.0f && ( IRelationType( tr.m_pEnt ) > D_FR || InSameTeam( tr.m_pEnt ) ) )
		return 0;
...
bool CNPC_Antlion::IsValidEnemy( CBaseEntity *pEnemy )
{
	...
	if ( IsAllied() && pEnemy->IsPlayer() && pEnemy->InSameTeam( this ) )
		return false;
