...
void CNPC_AttackHelicopter::Spawn( void )
{
	...
	int iHealth = sk_helicopter_health.GetInt();
	if ( lfe_npc_boss_addhealth_per_player.GetBool() )
	{
		...
		if ( iNumPlayers > iMinPlayers )
			iHealth += 10 * ( iNumPlayers - iMinPlayers );
...
int CNPC_AttackHelicopter::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	if ( pGren && info.GetAttacker() && info.GetAttacker()->IsPlayer() )
	{
		...
		else if( TFGameRules()->GetSkillLevel() >= SKILL_HARD )
		{
...
void CNPC_AttackHelicopter::Event_Killed( const CTakeDamageInfo &info )
{
	...
	m_OnDeath.FireOutput( pKiller, this );
}
