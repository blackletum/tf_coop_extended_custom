...
LINK_ENTITY_TO_CLASS( prop_vehicle_passengerseat, CPropVehiclePassengerSeat );
...
void CPropVehiclePassengerSeat::Precache(void)
{
	...
	PrecacheModel( "models/jeep_seat.mdl" );
...
void CPropVehiclePassengerSeat::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( pPlayer->GetVehicleEntity() != this )
		m_ServerVehicle.HandlePassengerEntry( pPlayer, (value>0) );
	else
...
// Purpose: 
//-----------------------------------------------------------------------------
void CPropVehiclePassengerSeat::UpdateOnRemove( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CPassengerSeatServerVehicle::HandlePassengerEntry( CBaseCombatCharacter *pPassenger, bool bAllowEntryOutsideZone )
{
	CBasePlayer *pPlayer = ToBasePlayer( pPassenger );
	...
	BaseClass::HandlePassengerEntry( pPassenger, bAllowEntryOutsideZone );
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
bool CPassengerSeatServerVehicle::HandlePassengerExit( CBaseCombatCharacter *pPassenger )
{
	CBasePlayer *pPlayer = ToBasePlayer( pPassenger );
	if ( !pPlayer )
		return false;

	pPlayer->SwitchToNextBestWeapon( pPassenger->GetActiveWeapon() );
	pPlayer->LeaveVehicle();
	return true;
}
...
void CPassengerSeatServerVehicle::ItemPostFrame( CBasePlayer* player )
{
	...
	if ( player->m_afButtonPressed & IN_USE || player->m_afButtonPressed & IN_JUMP )
	{
		if ( GetDrivableVehicle()->CanExitVehicle( player ) )
		{
			if ( !HandlePassengerExit( player ) )
			{
