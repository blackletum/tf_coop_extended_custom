...
void CNPC_Citizen::Spawn()
{
	...
	m_iMaxHealth = m_iHealth = (m_Type == CT_REBEL) ? (sk_citizen_health.GetInt() * 1.5f) : sk_citizen_health.GetInt();
...
void CNPC_Citizen::PickupItem( CBaseEntity *pItem )
{
	...
	else if( FClassnameIs( pItem, "item_healthkit_full" ) )
	{
		if ( TakeHealth( GetMaxHealth(), DMG_GENERIC ) )
		{
			RemoveAllDecals();
			UTIL_Remove( pItem );
		}
	}
	else if( FClassnameIs( pItem, "item_healthkit_medium" ) )
	{
		if ( TakeHealth( GetMaxHealth() / 2, DMG_GENERIC ) )
		{
			RemoveAllDecals();
			UTIL_Remove( pItem );
		}
	}
	else if( FClassnameIs( pItem, "item_healthkit_small" ) )
	{
		if ( TakeHealth( GetMaxHealth() / 3, DMG_GENERIC ) )
		{
			RemoveAllDecals();
			UTIL_Remove( pItem );
		}
...
{
	if ( ( ( info.GetDamageType() & DMG_BURN ) || (info.GetDamageType() & DMG_IGNITE ) ) && ( info.GetDamageType() & DMG_DIRECT ) )
	{
...
bool CNPC_Citizen::CanJoinPlayerSquad( CBasePlayer *pPlayer )
{
	...
	if ( V_strnicmp( STRING( gpGlobals->mapname ), "d1_trainstation_", 16 ) == 0 )
		return false;
...
CAI_BaseNPC *CNPC_Citizen::GetSquadCommandRepresentative()
{
	...
	if ( IsInPlayerSquad() )
	{
		...
		{
			Assert( dynamic_cast<CNPC_Citizen *>(m_hCurrentSquadRep.Get()) && m_hCurrentSquadRep->IsInPlayerSquad() );
