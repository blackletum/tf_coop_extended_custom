...
extern ConVar lfe_npc_boss_addhealth_per_player;
extern ConVar lfe_npc_boss_min_player_count;
...
{
	IGameEvent *event = gameeventmanager->CreateEvent( "antlionguard_killed" );
	if ( event )
	{
...
void CNPC_AntlionGuard::Spawn( void )
{
	...
	int iHealth = sk_antlionguard_health.GetInt();
	if ( lfe_npc_boss_addhealth_per_player.GetBool() )
	{
		int iNumPlayers = GetGlobalTFTeam( TF_TEAM_RED )->GetNumPlayers();
		int iMinPlayers = lfe_npc_boss_min_player_count.GetInt();
		if ( iNumPlayers > iMinPlayers )
			iHealth += 35 * ( iNumPlayers - iMinPlayers );
		m_iHealth = m_iMaxHealth = iHealth;
	}

	m_iMaxHealth = m_iHealth = iHealth;
	...
	AddCustomAttribute( "airblast vulnerability multiplier", 0.5 );
}
...
int CNPC_AntlionGuard::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if ( TFGameRules()->GetSkillLevel() >= SKILL_HARD && !(info.GetDamageType() & DMG_CRUSH) )
	{
