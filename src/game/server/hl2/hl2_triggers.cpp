...
void CTriggerPhysicsTrap::Touch( CBaseEntity *pOther )
{
	...
	if ( FClassnameIs( pAnim, "tf_weapon_physcannon" ) || FClassnameIs( pAnim, "weapon_physcannon" ) )
	{
...
void CTriggerWateryDeath::StartTouch(CBaseEntity *pOther)
{
	...
	if ( pOther->IsPlayer() )
	{
		...
		if ( pTFPlayer )
			pTFPlayer->StartWaterDeathSounds();
	}
}
...
void CTriggerWateryDeath::EndTouch( CBaseEntity *pOther )
{
	...
	if ( pOther->IsPlayer() )
	{
		...
		if ( pTFPlayer )
		{
			...
			if ( !(pTFPlayer->GetFlags() & FL_DONTTOUCH ) )
				  pTFPlayer->StopWaterDeathSounds();
		}
	}
