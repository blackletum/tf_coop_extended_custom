...
#ifdef TF_CLASSIC
		CBasePlayer *pPlayer = AI_GetNearestPlayer( GetEnemy()->GetAbsOrigin() );
#else
		CBasePlayer *pPlayer = AI_GetSinglePlayer();
#endif
...
void CNPC_Vortigaunt::Spawn( void )
{
	...
	m_iMaxHealth = m_iHealth = sk_vortigaunt_health.GetFloat();
...
void CNPC_Vortigaunt::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( HealBehaviorAvailable() )
	{
		...
		if ( pHealTarget != NULL && pHealTarget->GetHealth() < 0.4 )
		{
...
bool CNPC_Vortigaunt::ShouldHealTarget( CBaseEntity *pTarget )
{
	...
	// Make sure we're in same team as the target
