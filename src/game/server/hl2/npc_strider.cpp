...
void CNPC_Strider::Spawn()
{
	...
	AddFlag( FL_FLY );
	SetCollisionGroup( COLLISION_GROUP_NPC /*HL2COLLISION_GROUP_STRIDER*/ );
	SetSolid( SOLID_BBOX );
...
int CNPC_Strider::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	if ( ( info.GetAttacker()->IsPlayer() ) && m_takedamage != DAMAGE_EVENTS_ONLY )
	{
		if ( info.GetDamageCustom() == TF_DMG_CUSTOM_CANNONBALL_PUSH )
		{
