...
private:
	...
	CHandle< CPhysicsProp >		m_hExtraSeat;
...
void CPropAirboat::Spawn( void )
{
	...
	int nAttachment = LookupAttachment( "vehicle_driver_eyes" );
	if ( nAttachment )
	{
		...
		if ( m_hExtraSeat )
		{
			...
			m_hExtraSeat->SetLocalOrigin( Vector( -32, 0, 20 ) );
		}
	}
}
...
void CPropAirboat::FireGun( )
{
	...
	*/
	...
	if ( pEnt && (pEnt->GetHealth() > 0) && !InSameTeam( pEnt ) )
	{
