...
IMPLEMENT_SERVERCLASS_ST( CGrenadeFrag, DT_GrenadeFrag )
...
void CGrenadeFrag::Explode( trace_t *pTrace, int bitsDamageType )
{
	...
	Vector vecReported = pAttacker ? pAttacker->GetAbsOrigin() : vec3_origin;
	...
	if ( pNPC )
	{
		pNPC->CalcIsAttackMiniCritical();
		if ( pNPC->IsCurrentAttackAMiniCrit() )
		{
			bitsDamageType |= DMG_MINICRITICAL;
		}
	}


	CTakeDamageInfo info( this, pAttacker, GetBlastForce(), GetAbsOrigin(), m_flDamage, bitsDamageType, 0, &vecReported );
...
void CGrenadeFrag::Deflected( CBaseEntity *pDeflectedBy, Vector &vecDir )
{
	...
	if ( pPhysicsObject )
	{
		...
		pPhysicsObject->SetVelocityInstantaneous( &vecVelocity, NULL );
	}*/
...
CBaseGrenade *Fraggrenade_Create( const Vector &position, const QAngle &angles, const Vector &velocity, const AngularImpulse &angVelocity, CBaseEntity *pOwner, float timer, bool combineSpawned )
{
	...
	pGrenade->SetCritical( ((CAI_BaseNPC *)pOwner)->IsCurrentAttackACrit() );
