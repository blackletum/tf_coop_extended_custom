...
void CBaseHeadcrab::Spawn( void )
{
	...
	SetCollisionGroup( COLLISION_GROUP_NPC /*HL2COLLISION_GROUP_HEADCRAB*/ );
...
void CBaseHeadcrab::LeapTouch( CBaseEntity *pOther )
{
	...
	if ( IRelationType( pOther ) <= D_FR || !InSameTeam( pOther ) )
	{
...
void CBaseHeadcrab::TouchDamage( CBaseEntity *pOther )
{
	...
	{
		if ( pTFPlayer )
			pTFPlayer->m_Shared.MakeBleed( NULL, NULL, 2.0f, 4 );
	}
}
...
void CBaseHeadcrab::GatherConditions( void )
{
	...
	if ( m_bHidden )
	{
		...
		trace_t tr;
		UTIL_TraceHull( GetAbsOrigin(), GetAbsOrigin(),GetHullMins(), GetHullMaxs(), MASK_SHOT, this, GetCollisionGroup(), &tr );
		if ( tr.fraction == 1.0 )
		{
...
int CBaseHeadcrab::RangeAttack1Conditions( float flDot, float flDist )
{
	...
	if( pEnemy )
	{
		...
		trace_t tr;
		UTIL_TraceLine( EyePosition(), pEnemy->EyePosition(), MASK_SOLID, this, COLLISION_GROUP_NONE, &tr );
		...
		if( GetEnemy()->EyePosition().z - 36.0f > GetAbsOrigin().z )
		{
			...
			UTIL_TraceHull( vStartHullTrace, vEndHullTrace,GetHullMins(), GetHullMaxs(), MASK_NPCSOLID, this, GetCollisionGroup(), &tr );
...
void CHeadcrab::Spawn( void )
{
	...
	m_iMaxHealth = m_iHealth = sk_headcrab_health.GetFloat();
	m_flBurrowTime = 0.0f;
	m_bCrawlFromCanister = false;
	m_bMidJump = false;
	...
	case TASK_HEADCRAB_HOP_ASIDE:
		{
			...
			UTIL_TraceHull( GetAbsOrigin(), GetAbsOrigin() + vecDir,GetHullMins(), GetHullMaxs(), MASK_SHOT, this, GetCollisionGroup(), &tr );
...
void CBlackHeadcrab::Spawn( void )
{
	...
	m_bPanicState = false;
	m_iMaxHealth = m_iHealth = sk_headcrab_poison_health.GetFloat();
