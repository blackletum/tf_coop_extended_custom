...
#define	FLOOR_TURRET_MODEL_SAPPED	"models/combine_turrets/floor_turret_sapper.mdl"
...
void CNPC_FloorTurret::Precache( void )
{
	...
	PrecacheModel( FLOOR_TURRET_MODEL_SAPPED );
...
void CNPC_FloorTurret::ActiveThink( void )
{
	...
	{
		NDebugOverlay::Cross3D( vecMid, -Vector(2,2,2), Vector(2,2,2), 0, 255, 0, false, 0.05 );
		NDebugOverlay::Cross3D( pEnemy->WorldSpaceCenter(), -Vector(2,2,2), Vector(2,2,2), 0, 255, 0, false, 0.05 );
		NDebugOverlay::Line( vecMid, pEnemy->WorldSpaceCenter(), 0, 255, 0, false, 0.05 );
	...
	if ( ( bEnemyVisible == false ) || ( flDistToEnemy > FLOOR_TURRET_RANGE ) || ( pTFEnemy && ( pTFEnemy->m_Shared.GetPercentInvisible() > 0.5 || ( pTFEnemy->m_Shared.InCond( TF_COND_DISGUISED ) && pTFEnemy->m_Shared.GetDisguiseTeam() == GetTeamNumber() ) ) ) )
	{
	...
	{
		//We want to look at the enemy's eyes so we don't jitter
		Vector	vecDirToEnemyEyes = pEnemy->WorldSpaceCenter() - vecMid;
		VectorNormalize( vecDirToEnemyEyes );
...
void CNPC_FloorTurret::TippedThink( void )
{
	...
	if ( gpGlobals->curtime < m_flThrashTime )
	{
		if ( m_flShotTime < gpGlobals->curtime )
		{
			...
			else if ( !IsCitizenTurret() )	// Citizen turrets don't wildly fire
			{
				Vector vecMuzzle, vecMuzzleDir;
				UpdateMuzzleMatrix();
				MatrixGetColumn( m_muzzleToWorld, 0, vecMuzzleDir );
...
//-----------------------------------------------------------------------------
// Purpose: If we are being held by an ally player, give them credit for our kills.
//
//-----------------------------------------------------------------------------
...
int CNPC_FloorTurret::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	if ( info.GetDamageCustom() == TF_DMG_CUSTOM_PLASMA_CHARGED )
	{
		...
		SetThink( &CNPC_FloorTurret::ReturnToLife );
		SetNextThink( gpGlobals->curtime + 4.0f );
	}

	// Bullet damage reduces alternative health counter.
	// Once alt health reaches 0 take 20 dmg and alt health is reset.
	if ( ( newInfo.GetDamageType() & DMG_BULLET || newInfo.GetDamageType() & DMG_IGNITE) && m_takedamage != DAMAGE_EVENTS_ONLY )
	{
		m_iHealthAlt -= newInfo.GetDamage();
		if ( m_iHealthAlt <= 0 )
		{
			if ( newInfo.GetInflictor() && newInfo.GetAttacker() )
			TakeDamage( CTakeDamageInfo( newInfo.GetInflictor(), newInfo.GetAttacker(), 20, DMG_GENERIC ) );
			m_iHealthAlt = 100;
		}
		else
		{
...
QAngle CNPC_FloorTurret::PreferredCarryAngles( void )
{
	...
	CBasePlayer *pPlayer = UTIL_GetNearestPlayer( WorldSpaceCenter() );
...
void CNPC_FloorTurret::BreakThink( void )
{
	...
	if ( hl2_episodic.GetBool() )
		...
		DispatchParticleEffect( "ExplosionCore_buildings", vecOrigin, GetAbsAngles() );
...
void CNPC_FloorTurret::SelfDestruct( void )
{
	...
	else
	{
		...
		if ( m_hFizzleEffect != NULL )
		{
			...
			// Setup our basic parameters
			m_hFizzleEffect->KeyValue( "start_active", "1" );
			m_hFizzleEffect->KeyValue( "effect_name", "burningplayer_corpse" );
			m_hFizzleEffect->SetParent( this );
			m_hFizzleEffect->SetAbsOrigin( WorldSpaceCenter() + ( vecUp * 12.0f ) );
			DispatchSpawn( m_hFizzleEffect );
			m_hFizzleEffect->Activate();
...
//-----------------------------------------------------------------------------
bool CNPC_FloorTurret::InputWrenchHit( CTFPlayer *pPlayer, CTFWrench *pWrench, Vector vecHitPos )
{
	if ( !pPlayer )
		return false;
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CNPC_FloorTurret::OnWrenchHit( CTFPlayer *pPlayer, CTFWrench *pWrench, Vector vecHitPos )
{
	bool bRepair = false;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_FloorTurret::OnAttachSapper( void )
{
	//SetModel( FLOOR_TURRET_MODEL_SAPPED );
