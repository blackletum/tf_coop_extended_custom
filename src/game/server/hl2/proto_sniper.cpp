...
public:
	...
	virtual const char *GetLocalizeName() { return /*IsPlayerAllySniper() ? "npc_alyx" :*/ BaseClass::GetLocalizeName(); }
...
BEGIN_DATADESC( CProtoSniper )
	...
	DEFINE_FIELD( m_bPlayerFirstCallOut, FIELD_BOOLEAN ),
...
void CProtoSniper::LaserOn( const Vector &vecTarget, const Vector &vecDeviance )
{
	...
	if ( !m_bPlayerFirstCallOut )
	{
		for ( int iTeam = TF_TEAM_RED; iTeam < TF_TEAM_COUNT; ++iTeam )
		{
			if ( iTeam != GetTeamNumber() )
			{
				if ( GetGlobalTFTeam( iTeam )->GetNumPlayers() <= 12 )
				{
					TFGameRules()->HaveAllPlayersSpeakConceptIfAllowed( MP_CONCEPT_MVM_SNIPER_CALLOUT, iTeam );
					m_bPlayerFirstCallOut = true;
				}
			}
		}
	}
...
void CProtoSniper::Spawn( void )
{
	...
	m_iMaxHealth = m_iHealth = sk_combine_s_health.GetInt();
	m_flFieldOfView		= 0.2;
...
void CProtoSniper::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if ( pKillerWeapon )
	{
		CTFSword *pSword = dynamic_cast<CTFSword *>( pKillerWeapon );
		if ( pSword )
...
#define MAX_TRIES	15
	for( int i = 0 ; i < MAX_TRIES ; i++ )
	{
		...
		{
			trace_t tr;
			UTIL_TraceLine(vecSrc, vecSrc + vecDir * 8192, MASK_SHOT, this, COLLISION_GROUP_NONE, &tr);
		...
		else
		{
			...
			for( int i = 0 ; i < NUM_STEPS ; i++ )
			{
				...
				if( UTIL_PointContents( vecCursor ) != CONTENTS_SOLID )
				{
					...
					// Fire another tracer.
					UTIL_TraceLine( vecCursor, vecCursor + m_vecDir * 8192, MASK_SHOT, this, COLLISION_GROUP_NONE, &tr );
