...
void CNPC_MetroPolice::Spawn( void )
{
	...
	//	DispatchParticleEffect( "bot_eye_glow", PATTACH_POINT_FOLLOW, this, "eyes" );
...
bool CNPC_MetroPolice::HasBaton( void )
{
	...
	if ( pWeapon )
	{
		...
		if ( pWeapon->CapabilitiesGet() & bits_CAP_WEAPON_MELEE_ATTACK1 )
			return true;
	}
...
bool CNPC_MetroPolice::BatonActive( void )
{
	...
	if ( FClassnameIs( pWeapon, "weapon_stunstick" ) )
	{
		CWeaponStunStick *pStick = dynamic_cast<CWeaponStunStick *>( pWeapon );
		if ( pStick )
			return pStick->GetStunState();
	}
	else if ( pWeapon->CapabilitiesGet() & bits_CAP_WEAPON_MELEE_ATTACK1 )
	{
		return true;
	}
