...
void CHLHealthKit::Spawn( void )
{
	...
	SetModelIndexOverride( VISION_MODE_PYRO, modelinfo->GetModelIndex( "models/items/medkit_medium_bday.mdl" ) );
	SetModelIndexOverride( VISION_MODE_HALLOWEEN, modelinfo->GetModelIndex( "models/items/halloween_medium_small.mdl" ) );
...
bool CHLHealthKit::MyTouch( CBasePlayer *pPlayer )
{
	...
	int iHealthToAdd = ceil( pTFPlayer->GetMaxHealth() * ( sk_healthkit.GetFloat() / 100 ) );
	CALL_ATTRIB_HOOK_INT_ON_OTHER( pTFPlayer, iHealthToAdd, mult_health_frompacks );
	int iHealthRestored = pTFPlayer->TakeHealth( iHealthToAdd, DMG_GENERIC );
	if ( iHealthRestored )
	{
		...
		{
			int iFakeHealthToAdd = ceil( pTFPlayer->m_Shared.GetDisguiseClass() * ( sk_healthkit.GetFloat() / 100 ) );
			...
			pTFPlayer->m_Shared.AddDisguiseHealth( iFakeHealthToAdd, false );
		}
...
public:
	...
	SetModelIndexOverride( VISION_MODE_HALLOWEEN, modelinfo->GetModelIndex( "models/items/halloween_medkit_small.mdl" ) );
	...
	bool MyTouch( CBasePlayer *pPlayer )
	{
		...
		int iHealthToAdd = ceil( pTFPlayer->GetMaxHealth() * ( sk_healthvial.GetFloat() / 100 ) );
		CALL_ATTRIB_HOOK_INT_ON_OTHER( pTFPlayer, iHealthToAdd, mult_health_frompacks );
		int iHealthRestored = pTFPlayer->TakeHealth( iHealthToAdd, DMG_GENERIC );
		if ( iHealthRestored )
		{
			...
			if ( pTFPlayer->m_Shared.InCond( TF_COND_DISGUISED ) )
			{
				int iFakeHealthToAdd = ceil( pTFPlayer->m_Shared.GetDisguiseClass() * ( sk_healthvial.GetFloat() / 100 ) );
				CTFPlayer *pDisguiseTarget = ToTFPlayer( pTFPlayer->m_Shared.GetDisguiseTarget() );
				CALL_ATTRIB_HOOK_INT_ON_OTHER( pDisguiseTarget, iFakeHealthToAdd, mult_health_frompacks );
				pTFPlayer->m_Shared.AddDisguiseHealth( iFakeHealthToAdd, false );
			}
