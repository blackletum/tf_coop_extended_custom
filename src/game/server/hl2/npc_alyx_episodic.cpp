...
void CNPC_Alyx::Spawn()
{
	...
	m_iMaxHealth = m_iHealth = sk_alyx_health_episodic.GetInt() * 2;
	...
	if ( pPlayer )
	{
		...
		if (bFlashlightState != m_bPlayerFlashlightState)
		{
			...
			else
			{
				...
				if ( pTFPlayer->m_Shared.GetFlashlightBattery() < 20 )
				{
...
void CNPC_Alyx::Weapon_Drop(CBaseCombatWeapon *pWeapon, const Vector *pvecTarget, const Vector *pVelocity)
{
	...
	if ( pWeapon && pWeapon->IsWeapon( TF_WEAPON_HL2_ALYXGUN ) )
		pWeapon->SUB_Remove();
...
void CNPC_Alyx::OnGivenWeapon(CBaseCombatWeapon *pNewWeapon)
{
	...
	if ( pNewWeapon->IsWeapon( TF_WEAPON_HL2_ALYXGUN ) )
	{
...
void CNPC_Alyx::Use(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value)
{
	...
	if ( TFGameRules()->IsInHL2EP1Map() || TFGameRules()->IsInHL2EP2Map() )
	{
		GetFollowBehavior().SetFollowTarget( NULL );
		GetFollowBehavior().SetFollowTarget( pActivator, true );
	}
}
...
CNPC_Alyx::WeaponType_t CNPC_Alyx::ComputeWeaponType(CBaseEntity *pWeapon)
{
	...
	CBaseCombatWeapon *pCombatWeapon = dynamic_cast<CBaseCombatWeapon*>( pWeapon );
	if ( pCombatWeapon )
	{
		if ( pCombatWeapon->IsPistol() )
			return WT_ALYXGUN;

		if ( pCombatWeapon->IsSMG() )
			return WT_SMG1;

		if ( pCombatWeapon->IsShotgun() )
			return WT_SHOTGUN;

		if ( pCombatWeapon->GetWeaponID() == TF_WEAPON_HL2_AR2 )
			return WT_AR2;
	}
	else
	{
