...
ConVar	sk_combine_crabsynth_health( "sk_combine_crabsynth_health", "0" );
...
void CCrabSynth::Spawn( void )
{
	if ( TFGameRules() && TFGameRules()->IsHL2Beta() )
	{
		...
		m_iHealth = sk_combine_crabsynth_health.GetFloat();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCrabSynth::TraceAttack( const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
