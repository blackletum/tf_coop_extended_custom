...
{ 
	...
	if ( NULL_STRING == m_strItemClass )
	{
		Warning( "CItem_ItemCrate(%i):  CRATE_SPECIFIC_ITEM with NULL ItemClass string!!!\n", entindex() );
		return;
	}
...
//		vecActualVelocity += vecVelocity;
		...
		{
			DispatchParticleEffect( "mvm_pow_gold_seq_firework_mid", GetAbsOrigin(), GetAbsAngles() );
			EmitSound( "ui.cratesmash_ultrarare_short" );
		}
	}
}
