...
{
	if ( V_strnicmp( STRING( gpGlobals->mapname ), "d1_trainstation_01", 18 ) == 0 || V_strnicmp( STRING( gpGlobals->mapname ), "d1_trainstation_02", 18 ) == 0 || V_strnicmp( STRING( gpGlobals->mapname ), "d1_trainstation_03", 18 ) == 0 || V_strnicmp( STRING( gpGlobals->mapname ), "d1_trainstation_04", 18 ) == 0   )
		return false;
...
void CNPC_BaseScanner::Deflected( CBaseEntity *pDeflectedBy, Vector &vecDir )
{
	...
	if ( VPhysicsGetObject() )
	{
		AngularImpulse torque = vecDir * 1000.0f;
		VPhysicsGetObject()->ApplyTorqueCenter( torque );
		VPhysicsGetObject()->ApplyForceCenter( vecDir * 512.0f );
	}
