...
int CNPC_CeilingTurret::OnTakeDamage( const CTakeDamageInfo &inputInfo )
{
	...
	{
		Disable();
		SetThink( &CNPC_CeilingTurret::Enable );
...
void CNPC_CeilingTurret::Shoot( const Vector &vecSrc, const Vector &vecDirToEnemy )
{
	...
	FireBullets( info );
	EmitSound( "NPC_FloorTurret.ShotSounds" ); //EmitSound( "NPC_CeilingTurret.ShotSounds" );
	DoMuzzleFlash();
}
