...
void CPropAPC::Event_Killed( const CTakeDamageInfo &info )
{
	...
	CBaseEntity *pAssister = TFGameRules()->GetAssister( this, pKiller, pInflictor );
	...
	if ( event )
	{
		...
		event->SetString( "victim_entname", STRING( GetEntityName() ) );
		event->SetInt( "victim_team", GetTeamNumber() );
...
int CPropAPC::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	if ( pWeapon && pAttacker != this )
	{
		...
		// Notify the damaging weapon.
		pWeapon->ApplyOnHitAttributes( this, pTFAttacker, dmgInfo );
