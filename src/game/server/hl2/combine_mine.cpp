...
// Purpose: 
//-----------------------------------------------------------------------------
void CBounceBomb::Deflected( CBaseEntity *pDeflectedBy, Vector &vecDir )
{
	IPhysicsObject *pPhysicsObject = VPhysicsGetObject();
	if ( pPhysicsObject )
	{
		...
		pPhysicsObject->ApplyForceCenter( vecVelocity );
	}

	EmitSound( "Weapon_FlameThrower.AirBurstAttackDeflect" );
	IncremenentDeflected();
	m_hDeflectOwner = ToBasePlayer( pDeflectedBy );
...
//-----------------------------------------------------------------------------
void CBounceBomb::IncremenentDeflected( void )
{
	m_iDeflected++;
}
