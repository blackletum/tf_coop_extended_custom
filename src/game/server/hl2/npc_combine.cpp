	...
	case TASK_ANNOUNCE_ATTACK:
		{
			...
			if ((int)pTask->flTaskData == 1)
			{
					...
					(gpGlobals->curtime - m_flLastAttackTime > 3.0) )
				{
					...
					m_Sentences.Speak( GetAnnouncedSentece(), SENTENCE_PRIORITY_HIGH );
			...
			{
				m_Sentences.Speak( GetGrenadeSentece(), SENTENCE_PRIORITY_MEDIUM );
				SetActivity(ACT_IDLE);
			...
			if (GetHintGroup() == NULL_STRING)
			{
				...
				if ( pEntity )
				{
					...
					if ( !HasMemory(bits_MEMORY_PLAYER_HURT) && pEntity->IsPlayer() || pEntity->IsBaseObject() && pEntity->GetHealth() <= 40 )
					{
						...
						m_Sentences.Speak( GetPlayerHitSentece(), SENTENCE_PRIORITY_INVALID );
...
void CNPC_Combine::AnnounceAssault(void)
{
	...
	if ( FVisible( pBCC ) )
	{
		m_Sentences.Speak( GetAssaultSentece() );
	}
}
...
{
	const char *pSentenceName = GetMonsterSentece();
	switch ( pEnemy->Classify() )
	{
	case CLASS_PLAYER:
		pSentenceName = GetPlayerSentece();
		break;
	...
	case CLASS_VORTIGAUNT:
		pSentenceName = GetRebelsSentece();
		break;

	case CLASS_PLAYER_ALLY_VITAL:
		pSentenceName = GetVitalAllySentece();
		break;

	case CLASS_ANTLION:
		pSentenceName = GetAntlionSentece();
		break;

	case CLASS_ZOMBIE:
		pSentenceName = GetZombieSentece();
		break;

	case CLASS_HEADCRAB:
	case CLASS_BARNACLE:
		pSentenceName = GetHeadCrabsSentece();
		break;
	}
...
void CNPC_Combine::AnnounceEnemyKill( CBaseEntity *pEnemy )
{
	...
	const char *pSentenceName = GetGenericKillSentece();
	switch ( pEnemy->Classify() )
	{
	case CLASS_PLAYER:
		pSentenceName = GetPlayerKilledSentece();
		break;
			...
			if ( IsElite() )
			{
				if ( FVisible( m_hForcedGrenadeTarget ) )
				{
					if ( GetActiveWeapon() )
					{
						...
						if ( GetActiveWeapon()->IsWeapon( TF_WEAPON_HL2_AR2 ) )
						{
							return SCHED_COMBINE_AR2_ALTFIRE;
						}
						else if ( GetActiveWeapon()->IsWeapon( TF_WEAPON_HL2_SMG1 ) )
						{
			...
			if ( HasCondition(COND_HEAR_DANGER) )
			{
				...
				if ( pSound )
				{
					...
					if (pSound->m_iType & SOUND_DANGER && ( !IsInvulnerable() || !IsCritBoosted() ) )
					{
						...
						const char *pSentenceName = GetDangerSentece();
						...
						if ( pSoundOwner )
						{
							...
							if ( pGrenade && pGrenade->GetThrower() )
							{
								if ( IRelationType( pGrenade->GetThrower() ) != D_LI )
								{
									...
									pSentenceName = GetDangerGrenadeSentece();
								}
							}
						}
...
int CNPC_Combine::SelectScheduleAttack()
{
	...
	if ( GetEnemy() && GetEnemy()->IsBaseObject() && !InSameTeam( GetEnemy() ) )
	{
		...
		{
			bool bFirstContact = false;
			float flTimeSinceFirstSeen = gpGlobals->curtime - GetEnemies()->FirstTimeSeen( pObject );
			...
			if ( pObject->IsDisabled() && OccupyStrategySlot( SQUAD_SLOT_GRENADE1 ) )
				return SCHED_COMBINE_CHARGE_TURRET;
		}
	...
	case SCHED_TAKE_COVER_FROM_ENEMY:
		{
			if ( m_pSquad )
			{
				...
				if ( TFGameRules()->GetSkillLevel() <= SKILL_HARD && 
					HasCondition(COND_CAN_RANGE_ATTACK2)		&&
					OccupyStrategySlot( SQUAD_SLOT_GRENADE1 ) )
				{
					m_Sentences.Speak( GetTrownGrenadeSentece() );
	...
	case SCHED_RANGE_ATTACK1:
		{
			...
			if( GetActiveWeapon() && CanAltFireEnemy(true) && OccupyStrategySlot(SQUAD_SLOT_SPECIAL_ATTACK) )
			{
		...
		case COMBINE_AE_KICK:
			{
				...
				m_Sentences.Speak( GetKickSentece() );
				handledEvent = true;
				break;
			}

		case COMBINE_AE_CAUGHT_ENEMY:
			m_Sentences.Speak( GetAlertSentece() );
			handledEvent = true;
			break;
...
void CNPC_Combine::SpeakSentence( int sentenceType )
{
	...
	case 1: // Flanking the player
		...
		{
			m_Sentences.Speak( GetFlankSentece() );
		}
		break;
	}
}
...
void CNPC_Combine::PainSound ( const CTakeDamageInfo &info )
{
	...
	{
		const char *pSentenceName = GetPainSentece();
...
void CNPC_Combine::LostEnemySound( void)
{
	...
	{
		pSentence = GetLostLongSentece();
	}
	else
	{
		pSentence = GetLostShortSentece();
	}
...
{
	m_Sentences.Speak( GetFoundEnemySentece(), SENTENCE_PRIORITY_HIGH );
}
...
void CNPC_Combine::AlertSound( void)
{
	...
	{
		m_Sentences.Speak( GetGoAlertSentece(), SENTENCE_PRIORITY_HIGH );
...
void CNPC_Combine::NotifyDeadFriend ( CBaseEntity* pFriend )
{
	...
	{
		m_Sentences.Speak( GetLastManSentece(), SENTENCE_PRIORITY_INVALID, SENTENCE_CRITERIA_NORMAL );
		JustMadeSound();
		return;
	}
	...
	{
		m_Sentences.Speak( GetManDownSentece() );
	}
...
void CNPC_Combine::DeathSound ( void )
{
	...
	m_Sentences.Speak( GetDieSentece(), SENTENCE_PRIORITY_INVALID, SENTENCE_CRITERIA_ALWAYS );
}
...
void CNPC_Combine::IdleSound( void )
{
	if (g_fCombineQuestion || random->RandomInt(0,1))
	{
		if (!g_fCombineQuestion)
		{
			...
			{
			case 0: // check in
				if ( m_Sentences.Speak( GetCheckSentece() ) >= 0 )
				{
			...
			case 1: // question
				if ( m_Sentences.Speak( GetQuestSentece() ) >= 0 )
				{
			...
			case 2: // statement
				m_Sentences.Speak( GetIdleSentece() );
				break;
			}
		}
		else
		{
			...
			{
			case 1: // check in
				if ( m_Sentences.Speak( GetClearSentece() ) >= 0 )
				{
					g_fCombineQuestion = 0;
				}
				break;
			case 2: // question 
				if ( m_Sentences.Speak( GetAnswerSentece() ) >= 0 )
				{
					g_fCombineQuestion = 0;
				}
				break;
...
{
	if( pWeapon->GetWeaponID() == TF_WEAPON_HL2_AR2 )
	{
