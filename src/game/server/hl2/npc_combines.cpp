...
void CNPC_CombineS::Spawn( void )
{
	...
	{
		DevMsg( "Soldier %s is set to use march anim, but is not an efficient AI. The blended march anim can only be used for dead-ahead walks!\n", GetDebugName() );
	}
#endif
}
...
void CNPC_CombineS::Precache()
{
	...
	if( !Q_stricmp( pModelName, "models/combine_super_soldier.mdl" ) || !Q_stricmp( pModelName, "models/combine_super_soldier_ep2.mdl" ) )
	{
		m_bIsElite = true;
	}
	else
...
void CNPC_CombineS::DeathSound( const CTakeDamageInfo &info )
{
	...
	GetSentences()->Speak( GetDieSentece(), SENTENCE_PRIORITY_INVALID, SENTENCE_CRITERIA_ALWAYS ); 
}
