...
//
ConVar	sk_zombie_flinch_delay( "sk_zombie_flinch_delay","3");
...
int CNPC_BaseZombie::MeleeAttack1Conditions( float flDot, float flDist )
{
	...
	if( tr.m_pEnt == GetEnemy() || 
		tr.m_pEnt->IsNPC() || tr.m_pEnt->IsBaseObject() || 
...
bool CNPC_BaseZombie::ShouldBecomeTorso( const CTakeDamageInfo &info, float flDamageThreshold )
{
	...
	if ( ( LastHitGroup() == HITGROUP_CHEST || LastHitGroup() == HITGROUP_STOMACH ) && flDamageThreshold >= 0.8 )
		return true;
		...
		case RELEASE_RAGDOLL_SLICED_OFF:
			{
				...
				SpawnBlood( WorldSpaceCenter(), g_vecAttackDir, BloodColor(), inputInfo.GetDamage() );
...
bool CNPC_BaseZombie::IsChopped( const CTakeDamageInfo &info )
{
	...
	if ( info.GetWeapon() )
	{
		pWeapon = dynamic_cast<CTFWeaponBase *>( info.GetWeapon() );
	}
	...
	if ( pWeapon && flDamageThreshold >= 0.5 )
	{
		if ( pWeapon->GetWeaponID() == TF_WEAPON_BONESAW )
			return true;
...
CBaseEntity *CNPC_BaseZombie::ClawAttack( float flDist, int iDamage, QAngle &qaViewPunch, Vector &vecVelocityPunch, int BloodOrigin  )
{
	...
	if ( pHurt )
	{
		...
		if ( pPlayer != NULL && !(pPlayer->GetFlags() & FL_GODMODE ) )
		{
			...
			{
				if ( pTFPlayer )
					pTFPlayer->m_Shared.MakeBleed( NULL, NULL, 4.0f, 4 );
			}
		}
...
void CNPC_BaseZombie::OnScheduleChange( void )
{
	...
	if ( HasCondition( COND_LIGHT_DAMAGE ) || HasCondition( COND_HEAVY_DAMAGE ))
		m_flNextFlinch = gpGlobals->curtime + sk_zombie_flinch_delay.GetFloat();
