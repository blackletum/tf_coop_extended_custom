		...
		case TASK_HUNTER_CHARGE:
		{
			...
			if ( eActivity == ACT_HUNTER_CHARGE_STOP || eActivity == ACT_HUNTER_CHARGE_CRASH ) 
			{
				if ( IsActivityFinished() )
				{
					...
					float delayMultiplier = ( TFGameRules()->IsSkillLevel( SKILL_EASY ) ) ? 1.5 : 1.0;
...
bool CNPC_Hunter::HandleChargeImpact( Vector vecImpact, CBaseEntity *pEntity )
{
	...
	if ( ( IRelationType( pEntity ) <= D_FR || !InSameTeam( pEntity ) ) && ( GetNextAttack() < gpGlobals->curtime ) )
	{
...
int CNPC_Hunter::MeleeAttack1Conditions( float flDot, float flDist )
{
	...
	if ( tr.m_pEnt == GetEnemy() || tr.m_pEnt->IsNPC() || tr.m_pEnt->IsBaseObject() || (tr.m_pEnt->m_takedamage == DAMAGE_YES && (dynamic_cast<CBreakableProp*>(tr.m_pEnt))) )
	{
...
{
	float delayMultiplier = ( TFGameRules()->IsSkillLevel( SKILL_EASY ) ) ? 1.25 : 1.0;
