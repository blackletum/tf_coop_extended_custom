...
DEFINE_FIELD(m_hExtraSeat, FIELD_EHANDLE),
...
void CPropJeepEpisodic::Spawn(void)
{
	...
	int nAttachment = LookupAttachment( "vehicle_passenger_feet0" );
	if ( FStrEq( STRING( GetModelName() ), "models/buggy.mdl" ) )
		nAttachment = LookupAttachment( "seagull_perch3" );
	else if ( FStrEq( STRING( GetModelName() ), "models/vehicle.mdl" ) )
		nAttachment = LookupAttachment( "cargo" );

	if ( nAttachment )
	{
		Vector vecAttachOrigin;
		GetAttachment( nAttachment, vecAttachOrigin );

		m_hExtraSeat = (CPhysicsProp *)CreateEntityByName( "prop_vehicle_passengerseat" );
		if ( m_hExtraSeat )
		{
			m_hExtraSeat->SetAbsOrigin( vecAttachOrigin - Vector( 0, 0, 4 ) );
			m_hExtraSeat->SetAbsAngles( GetAbsAngles() );
			m_hExtraSeat->SetModel( "models/jeep_seat.mdl" );
			m_hExtraSeat->KeyValue( "solid", "6" );
			m_hExtraSeat->KeyValue( "vehiclescript", "scripts/vehicles/prisoner_pod.txt" );
			m_hExtraSeat->SetOwnerEntity( this );
			DispatchSpawn( m_hExtraSeat );
			m_hExtraSeat->Activate();
			m_hExtraSeat->VPhysicsDestroyObject();
			m_hExtraSeat->SetMoveType( MOVETYPE_NONE );
			m_hExtraSeat->SetParent( this );
			if ( FStrEq( STRING( GetModelName() ), "models/vehicle.mdl" ) )
				m_hExtraSeat->SetLocalAngles( QAngle( 0, 180, 0 ) );
		}
	}
}
...
void CPropJeepEpisodic::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( pActivator->IsPlayer() )
		ResetUseKey( ToBasePlayer( pActivator ) );
}
...
{
	if ( m_pServerVehicle->GetVehicleParams() == nullptr )
		return;
