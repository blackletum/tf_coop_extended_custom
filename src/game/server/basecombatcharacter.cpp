...
BEGIN_DATADESC( CBaseCombatCharacter )
	...
	DEFINE_FIELD( m_bIsMiniBoss, FIELD_BOOLEAN ),
	DEFINE_FIELD( m_bUseBossHealthBar, FIELD_BOOLEAN ),
...
IMPLEMENT_SERVERCLASS_ST(CBaseCombatCharacter, DT_BaseCombatCharacter)
	...
	SendPropDataTable( SENDINFO_DT( m_Shared ), &REFERENCE_SEND_TABLE( DT_BaseCombatCharacterShared ) ),
#ifdef INVASION_DLL
...
#if defined( HL1_DLL ) && !defined( TF_CLASSIC )
...
void CBaseCombatCharacter::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if( !FStrEq(STRING(gpGlobals->mapname), "d1_trainstation_01" ) || !FStrEq(STRING(gpGlobals->mapname), "d1_trainstation_02" ) || !FStrEq(STRING(gpGlobals->mapname), "d1_trainstation_03" ) || !FStrEq(STRING(gpGlobals->mapname), "d1_trainstation_04" ) )
	{
	...
	if ( ShouldGib( info ) == false )
	{
		...
		if ( ( ( info.GetDamageType() & DMG_DISSOLVE) || info.GetDamageCustom() == TF_DMG_CUSTOM_PLASMA ) && CanBecomeRagdoll() )
		{
...
int CBaseCombatCharacter::IRelationPriority( CBaseEntity *pTarget )
{
	if ( pTarget )
	{
		...
		if ( pTarget->IsBaseObject() )
		{
			...
			if ( pObject )
			{
				// everyone hated sentry because it beep the truth.
				if ( pObject->GetType() == OBJ_SENTRYGUN )
					return 10;
				else if ( pObject->GetType() == OBJ_DISPENSER || pObject->GetType() == OBJ_TELEPORTER )
					return 0;
			}
		}
...
{
	// Bypass stress completely for teammates.
	CBaseEntity *pEnt = static_cast<CBaseEntity*>( pPhysics->GetGameData() );
	if( pEnt && InSameTeam( pEnt ) )
		return;
...
//-----------------------------------------------------------------------------
void CBaseCombatCharacter::EquipWearable( CEconWearable *pItem )
{
	Assert( pItem );
...
//-----------------------------------------------------------------------------
void CBaseCombatCharacter::RemoveWearable( CEconWearable *pItem )
{
	Assert( pItem );
...
// Purpose: 
//-----------------------------------------------------------------------------
/*void CBaseCombatCharacter::PlayWearableAnimsForPlaybackEvent( wearableanimplayback_t iPlayback )
{
