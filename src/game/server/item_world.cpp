...
BEGIN_DATADESC( CItem )
	...
	DEFINE_KEYFIELD( m_iszPickupSound,	FIELD_SOUNDNAME,	"pickup_sound" ),
	DEFINE_KEYFIELD( m_iszPickupParticle,	FIELD_STRING,	"pickup_particle" ),
	...
	DEFINE_OUTPUT( m_OnGreenPickup, "OnGreenPickup" ),
	DEFINE_OUTPUT( m_OnYellowPickup, "OnYellowPickup" ),
...
void CItem::Spawn( void )
{
	...
	if ( m_iszPowerupModel != NULL_STRING )
	{
		SetModel( STRING( m_iszPowerupModel ) );
		SetModelIndexOverride( VISION_MODE_NONE, modelinfo->GetModelIndex( STRING( m_iszPowerupModel ) ) );
	}
...
void CItem::Precache()
{
	...
	PrecacheParticleSystem( STRING( m_iszPickupParticle ) );
