...
void CBasePlayer::PostThink()
{
	...
	#if defined( PORTAL ) // Portalgun view model stays up when holding an object -Jeep
					|| FClassnameIs( GetActiveWeapon(), "tf_weapon_portalgun" ) 
	#endif //#ifdef PORTAL			
					) )
				{  
...
// Create an airboat in front of the specified player
//-----------------------------------------------------------------------------
static void CreatePrisonerPod( CBasePlayer *pPlayer )
{
	// Cheat to create a citadel pod in front of the player
	Vector vecForward;
	AngleVectors( pPlayer->EyeAngles(), &vecForward );
	...
	if ( pPod )
	{
		...
		pPod->KeyValue( "model", "models/vehicles/prisoner_pod.mdl" );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CC_CH_CreatePrisonerPod( void )
{
	CBasePlayer *pPlayer = UTIL_GetCommandClient();
	if ( !pPlayer )
		return;

	CreatePrisonerPod( pPlayer );
}

static ConCommand ch_createprisonerpod( "ch_createprisonerpod", CC_CH_CreatePrisonerPod, "Spawn airboat in front of the player.", FCVAR_CHEAT );
...
bool CBasePlayer::ShouldAutoaim( void )
{
	...
	return ( IsX360() || TFGameRules()->GetSkillLevel() < SKILL_HARD );
}
