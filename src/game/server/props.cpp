...
void CBaseProp::Precache( void )
{
	...
	PrecacheParticleSystem( "peejar_impact_bits_milk" );
	...
	if( HasInteraction( PROPINTER_PHYSGUN_FIRST_PAINT ) )
	{
		...
		if ( tr.m_pEnt )
		{
			// lets paintsplat everything
			{
				...
				DispatchParticleEffect( "peejar_impact_bits_milk", vecPos, GetAbsAngles() );
			}
		}
	}
...
//-----------------------------------------------------------------------------
// Mapbase: Handles keyvalues from the BSP. Called before spawning.
//-----------------------------------------------------------------------------
...
void CBreakableProp::BreakablePropTouch( CBaseEntity *pOther )
{
	...
	if ( m_hFlareEnt )
	{
		...
		if ( pNPC && pNPC->AllowedToIgnite() && !pNPC->IsOnFire() && !pNPC->InSameTeam( m_hPhysicsAttacker ) )
		{
			pNPC->Ignite( 25.0f );
		...
		{
			pProp->IgniteLifetime( 25.0f );
			if ( pProp->GetEffectEntity() && HasPhysicsAttacker( PROP_FLARE_LIFETIME ) )
				pProp->GetEffectEntity()->SetOwnerEntity( m_hPhysicsAttacker );
			KillFlare( this, m_hFlareEnt, PROP_FLARE_IGNITE_SUBSTRACT );
			IGameEvent *event = gameeventmanager->CreateEvent( "flare_ignite_npc" );
			if ( event )
			{
				event->SetInt( "entindex", pProp->entindex() );
				gameeventmanager->FireEvent( event );
			}
		}
	}
...
int CBreakableProp::OnTakeDamage( const CTakeDamageInfo &inputInfo )
{
	...
	bool bTFIgniting = false;

	// Start burning if we took ignition damage
	bTFIgniting = ( ( info.GetDamageType() & DMG_IGNITE ) && ( GetWaterLevel() < WL_Waist ) );

	if ( !bTFIgniting )
	{
		int iIgniting = 0;
		CALL_ATTRIB_HOOK_FLOAT_ON_OTHER( info.GetWeapon(), iIgniting, set_dmgtype_ignite );
		bTFIgniting = ( iIgniting != 0 );
	}
	...
	else if( !bDeadly && ( ( info.GetDamageType() & DMG_BURN ) || bTFIgniting ) )
	{
...
//-----------------------------------------------------------------------------
// Mapbase: Input handler for setting interactions.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Mapbase: Input handler for Adding interactions.
//-----------------------------------------------------------------------------
...
void CBreakableProp::Break( CBaseEntity *pBreaker, const CTakeDamageInfo &info )
{
	...
	if ( info.GetDamageCustom() == TF_DMG_CUSTOM_GOLD_WRENCH )
	{
		MaterialOverride( "models/player/shared/gold_player.vmt" );
	...
	if ( info.GetDamageCustom() == TF_DMG_CUSTOM_PLASMA || info.GetDamageCustom() == TF_DMG_CUSTOM_PLASMA_CHARGED || info.GetDamageCustom() == TF_DMG_CUSTOM_PLASMA_GIB )
	{
...
//-----------------------------------------------------------------------------
// Mapbase: Button prop for +USEable dynamic props
//-----------------------------------------------------------------------------
