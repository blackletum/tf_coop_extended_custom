...
void CBaseServerVehicle::HandlePassengerEntry( CBaseCombatCharacter *pPassenger, bool bAllowEntryOutsideZone )
{
	...
	if ( pPlayer != NULL )
	{
		...
		if ( GetDrivableVehicle()->CanEnterVehicle( pPlayer ) )
		{
			...
			{
				int nRole = NPC_GetAvailableSeat( pPlayer, MAKE_STRING( "passenger" ), VEHICLE_SEAT_ANY );
				if ( nRole != VEHICLE_SEAT_INVALID )
				{
					NPC_AddPassenger( pPlayer, MAKE_STRING( "passenger" ), nRole );
...
int CBaseServerVehicle::GetExitAnimToUse( Vector &vecEyeExitEndpoint, bool &bAllPointsBlocked )
{
	...
	for ( int i = 0; i < iCount; i++ )
	{
		...
		Ray_t ray; ray.Init( vecViewOrigin, vecExitEndPoint );
		UTIL_Portal_TraceRay( ray, MASK_PLAYERSOLID, pAnimating, COLLISION_GROUP_NONE, &tr );
		if ( tr.fraction != 1.0 )
		{
#ifdef HL2_EPISODIC
...
void CBaseServerVehicle::ProcessMovement( CBasePlayer *pPlayer, CMoveData *pMoveData )
{
	...
	trace_t	tr;
	Ray_t ray; ray.Init( pPlayer->GetAbsOrigin(), pPlayer->GetAbsOrigin() - Vector( 0, 0, 256 ) );
	UTIL_Portal_TraceRay( ray, MASK_PLAYERSOLID, GetVehicleEnt(), COLLISION_GROUP_NONE, &tr );
