	...
	if ( bMakeSound )	
	{
		...
		for( int i = 0; i < iNumCappers; i++ )
		{
			...
			for ( int iTeam = FIRST_GAME_TEAM; iTeam < nTeamCount; ++iTeam )
			{
				...
				if ( pTeam )
				{
					CBasePlayer *pPlayer = pTeam->GetPlayer( playerIndex );
					if ( pPlayer )
						PlayerCapped( pPlayer );

					CAI_BaseNPC *pNPC = pTeam->GetNPC( playerIndex );
					if ( pNPC )
						PlayerCapped( pNPC );
				}
			}
...
void CTeamControlPoint::CaptureBlocked( CBaseCombatCharacter *pPlayer, CBaseCombatCharacter *pVictim )
{
	...
	if ( event )
	{
		...
		event->SetString( "blocker_name", pPlayer->GetClassname() );
		event->SetInt( "priority", 9 );
		if ( pVictim )
...
//-----------------------------------------------------------------------------
void CTeamControlPoint::PlayerCapped( CBaseCombatCharacter *pPlayer )
{
	CBaseMultiplayerPlayer *pMultiPlayer = ToBaseMultiplayerPlayer( pPlayer );
	if ( pMultiPlayer )
	{
		pMultiPlayer->SpeakConceptIfAllowed( MP_CONCEPT_CAPTURED_POINT );
	}
}
...
//-----------------------------------------------------------------------------
void CTeamControlPoint::PlayerBlocked( CBaseCombatCharacter *pPlayer )
{
	CBaseMultiplayerPlayer *pMultiPlayer = ToBaseMultiplayerPlayer( pPlayer );
	if ( pMultiPlayer )
	{
		pMultiPlayer->SpeakConceptIfAllowed( MP_CONCEPT_CAPTURE_BLOCKED );
	}
}
