...
static ClassNamePrefix_t s_pNPCEntityPrefixes[] =
{
	...
	ClassNamePrefix_t( "headless_", true ),
	ClassNamePrefix_t( "eyeball_", true ),
	ClassNamePrefix_t( "bot_npc_", true ),
};
...
CON_COMMAND_F_COMPLETION( npc_create, "Creates an NPC of the given type where the player is looking (if the given NPC can actually stand at that location).  Note that this only works for npc classes that are already in the world.  You can not create an entity that doesn't have an instance in the level.\n\tArguments:	{npc_class_name} {targetname} {health}", FCVAR_CHEAT, CreateNPCAutoComplete )
{
	...
	if ( baseNPC )
	{
		...
		if ( args.ArgC() == 4 )
		{
			baseNPC->SetMaxHealth( atof( args[3] ) );
		...
		UTIL_Portal_TraceRay( ray, baseNPC->GetAITraceMask(), pPlayer, COLLISION_GROUP_NONE, &tr );
	...
	if (baseNPC)
	{
		...
		if ( tr.fraction != 1.0 )
		{
			...
			else
			{
				// Raise the end position a little up off the floor, place the npc and drop him down
				tr.endpos.z += 12;
				if ( FStrEq( args[1], "npc_strider" ) || FStrEq( args[1], "npc_combinegunship" ) )
					tr.endpos.z += 128;
				else if ( FStrEq( args[1], "npc_turret_floor" ) || FStrEq( args[1], "npc_portal_turret_floor" ) )
					tr.endpos.z += 2;
				else
