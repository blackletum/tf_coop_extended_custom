...
void CPointServerCommand::InputCommand( inputdata_t& inputdata )
{
	...
	// Don't Allows this stuffs to be runned by default
	char const *comparename = inputdata.value.String();
	if ( V_strstr( comparename, "rcon" ) || 
		 V_strstr( comparename, "admin" ) || 
		 V_strstr( comparename, "sm" ) || 
		 V_strstr( comparename, "test_" ) || 
		 V_strstr( comparename, "alias" ) || 
		 V_strstr( comparename, "command" ) ||
		 V_strstr( comparename, "ban" ) || 
		 V_strstr( comparename, "kick" ) || 
		 V_strstr( comparename, "ip" ) || 
		 V_strstr( comparename, "cmd" ) )
	return;
...
void CC_God_f (void)
{
   ...
   if ( TFGameRules() && ( TFGameRules()->IsPVEModeActive() == false ) )
   {
	...
	if (!(pPlayer->GetFlags() & FL_GODMODE ) )
		...
		ConColorMsg( Color( 238, 103, 256, 255 ), "godmode OFF\n" );
	else
		//ClientPrint( pPlayer, HUD_PRINTCONSOLE, "godmode ON\n");
		ConColorMsg( Color( 238, 103, 256, 255 ), "godmode ON\n" );
}
...
void CC_Notarget_f (void)
{
	...
	if ( !(pPlayer->GetFlags() & FL_NOTARGET ) )
		...
		ConColorMsg( Color( 238, 103, 256, 255 ), "notarget OFF\n" );
	else
		//ClientPrint( pPlayer, HUD_PRINTCONSOLE, "notarget ON\n");
		ConColorMsg( Color( 238, 103, 256, 255 ), "notarget ON\n" );
}
