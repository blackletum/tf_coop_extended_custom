	...
	for ( int iTeam = FIRST_GAME_TEAM; iTeam < nTeamCount; ++iTeam )
	{
		...
		if ( pTeam )
		{
			for ( int i=0;i<pTeam->GetNumPlayers();i++ )
			{
				...
				if ( IsTouching( pPlayer ) && ( pPlayer->GetTeamNumber() == team ) ) // need to make sure disguised spies aren't included in the list of capping players
				{
			...
			for ( int iNPC= 0;iNPC<pTeam->GetNumNPCs();iNPC++ )
			{
				...
				if ( IsTouching( pNPC ) && ( pNPC->GetTeamNumber() == team ) ) // need to make sure disguised spies aren't included in the list of capping players
				{
					if ( numcappers < MAX_AREA_CAPPERS-1 )
					{
						cappingplayers[numcappers] = iNPC;
						numcappers++;
					}
				}
			}
