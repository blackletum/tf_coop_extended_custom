...
void CTFWeaponBuilder::StartPlacement(void)
{
	...
	if ( GetTFPlayerOwner() && GetTFPlayerOwner()->m_Shared.GetCarriedObject() )
	{
		m_hObjectBeingBuilt = GetTFPlayerOwner()->m_Shared.GetCarriedObject();
		m_hObjectBeingBuilt->StartPlacement( GetTFPlayerOwner() );
		return;
	}
...
void CTFWeaponSapper::ItemPostFrame( void )
{
	...
	int iWheatley = 0;
	CALL_ATTRIB_HOOK_INT( iWheatley, sapper_voice_pak );
	if ( iWheatley )
	{
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
int CTFWeaponSapper::TranslateViewmodelHandActivity( int iActivity )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CTFWeaponSapper::CanCharge( void )
{
	return false;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
float CTFWeaponSapper::GetChargeBeginTime( void )
{
	return 0.0f;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
float CTFWeaponSapper::GetChargeMaxTime( void )
{
	float flTime = 15.0f;
	CTFPlayer *pPlayer = GetTFPlayerOwner();
