...
ConVar bot_saveme( "bot_saveme", "0", FCVAR_REPLICATED );
static ConVar bot_mimic( "bot_mimic", "0", 0, "Bot uses usercmd of player by index." );
static ConVar bot_mimic_yaw_offset( "bot_mimic_yaw_offset", "180", 0, "Offsets the bot yaw." );
ConVar bot_selectweaponslot( "bot_selectweaponslot", "-1", FCVAR_REPLICATED, "set to weapon slot that bot should switch to." );
ConVar bot_randomnames( "bot_randomnames", "0", FCVAR_REPLICATED );
ConVar bot_jump( "bot_jump", "0", FCVAR_REPLICATED, "Force all bots to repeatedly jump." );
...
CON_COMMAND_F( bot, "Add a bot.", FCVAR_REPLICATED )
{
...
void cc_bot_sendcommand( const CCommand &args )
{
	...
	if ( FStrEq( args[1], "all" ) )
	{
		for ( int i = 1; i <= gpGlobals->maxClients; i++ )
		{
			CTFPlayer *pBots = ToTFPlayer( UTIL_PlayerByIndex( i ) );

			if ( pBots && pBots->IsFakeClient() )
			{
				TFGameRules()->ClientCommand( pBots, command );
			}
		}
	}
...
static ConCommand bot_sendcommand( "bot_command", cc_bot_sendcommand, "<bot id> <command string...>.  Sends specified command on behalf of specified bot", FCVAR_REPLICATED );
...
static ConCommand bot_kill( "bot_kill", cc_bot_kill, "<bot id>.  Kills bot.", FCVAR_REPLICATED );

//------------------------------------------------------------------------------
// Purpose: sends the specified command from a bot
//------------------------------------------------------------------------------
void cc_bot_kick( const CCommand &args )
{
	// get the bot's player object
	CBasePlayer *pPlayer = UTIL_PlayerByName( args[1] );
	...
	if ( pPlayer )
	{
		engine->ServerCommand( CFmtStr( "kickid %d %s;", pPlayer->GetUserID(), "Kicked by server." ) );
	}
	...
	if ( FStrEq( args[1], "all" ) )
	{
		for ( int i = 1; i <= gpGlobals->maxClients; i++ )
		{
			...
			if ( pBots && pBots->IsFakeClient() )
			{
				engine->ServerCommand( CFmtStr( "kickid %d %s;", pBots->GetUserID(), "Kicked by server." ) );
			}
		}
	}
}

static ConCommand bot_kick( "bot_kick", cc_bot_kick, "<bot id>.  Kick bot.", FCVAR_REPLICATED );

CON_COMMAND_F( bot_changeteams, "Make all bots change teams", FCVAR_REPLICATED )
{
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		CTFPlayer *pPlayer = ToTFPlayer( UTIL_PlayerByIndex( i ) );
...
CON_COMMAND_F( bot_refill, "Refill all bot ammo counts", FCVAR_REPLICATED )
{
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		CTFPlayer *pPlayer = ToTFPlayer( UTIL_PlayerByIndex( i ) );
...
CON_COMMAND_F( bot_whack, "Deliver lethal damage from player to specified bot", FCVAR_REPLICATED )
{
	if ( args.ArgC() < 2 )
	{
	...
	if ( FStrEq( args[1], "all" ) )
	{
		for ( int i = 1; i <= gpGlobals->maxClients; i++ )
		{
			...
			if ( pBots && pBots->IsFakeClient() )
			{
				pBots->TakeDamage( info );
			}
		}
	}
}

CON_COMMAND_F( bot_teleport, "Teleport the specified bot to the specified position & angles.\n\tFormat: bot_teleport <bot name> <X> <Y> <Z> <Pitch> <Yaw> <Roll>", FCVAR_REPLICATED )
{
	if ( args.ArgC() < 8 )
	{
	...
	if ( FStrEq( args[1], "all" ) )
	{
		for ( int i = 1; i <= gpGlobals->maxClients; i++ )
		{
			...
			if ( pBots && pBots->IsFakeClient() )
			{
				pBots->Teleport( &vecPos, &vecAng, NULL );
			}
		}
	}
}
...
// Purpose: sends the specified command from a bot
//------------------------------------------------------------------------------
void cc_bot_warp_team_to_me( const CCommand &args )
{
	...
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		if ( pBots && pBots->IsFakeClient() )
		{
			if ( pBots->InSameTeam( pPlayer ) )
				pBots->Teleport( &pPlayer->GetAbsOrigin(), &pPlayer->GetAbsAngles(), NULL );
		}
	}
}

static ConCommand bot_warp_team_to_me( "bot_warp_team_to_me", cc_bot_warp_team_to_me, "Warp bot.", FCVAR_REPLICATED );
