...
bool CObjectSentrygun::ValidTargetPlayer( CTFPlayer *pPlayer, const Vector &vecStart, const Vector &vecEnd )
{
	...
	// Ray trace!!!
	bool bVisible = FVisible( pPlayer, MASK_SHOT /*| CONTENTS_GRATE*/ );

	// If the enemy isn't in the normal fov, check the fov through portals
	CProp_Portal *pPortal = NULL;
	{
		pPortal = FInViewConeThroughPortal( pPlayer );

		if ( pPortal )
		{
			bVisible = FVisibleThroughPortal( pPortal, pPlayer, MASK_SHOT /*| CONTENTS_GRATE*/ );
		}
	}
...
bool CObjectSentrygun::ValidTargetObject( CBaseObject *pObject, const Vector &vecStart, const Vector &vecEnd )
{
	...
	// Ray trace.
	bool bVisible = FVisible( pObject, MASK_SHOT /*| CONTENTS_GRATE*/ );

	// If the enemy isn't in the normal fov, check the fov through portals
	CProp_Portal *pPortal = NULL;
	{
		pPortal = FInViewConeThroughPortal( pObject );

		if ( pPortal )
		{
			bVisible = FVisibleThroughPortal( pPortal, pObject, MASK_SHOT /*| CONTENTS_GRATE*/ );
		}
	}
...
bool CObjectSentrygun::ValidTargetNPC( CAI_BaseNPC *pNPC, const Vector &vecStart, const Vector &vecEnd )
{
	...
	// Ray trace?!
	bool bVisible = FVisible( pNPC, MASK_SHOT /*| CONTENTS_GRATE*/ );

	// If the enemy isn't in the normal fov, check the fov through portals
	CProp_Portal *pPortal = NULL;
	{
		pPortal = FInViewConeThroughPortal( pNPC );

		if ( pPortal )
		{
			bVisible = FVisibleThroughPortal( pPortal, pNPC, MASK_SHOT /*| CONTENTS_GRATE*/ );
		}
	}
...
void CObjectSentrygun::Attack()
{
	...
	// If the enemy isn't in the normal fov, check the fov through portals
	CProp_Portal *pPortal = FInViewConeThroughPortal( m_hEnemy );

	//We want to look at the enemy's eyes so we don't jitter
	Vector vEnemyWorldSpaceCenter = m_hEnemy->WorldSpaceCenter();
	...
	QAngle angToTarget;
	VectorAngles( vecDirToEnemyEyes, angToTarget );
...
bool CObjectSentrygun::Fire()
{
	...
	if ( m_iAmmoShells > 0)
	{
		...
		FireBullets( info );
		CSoundEnt::InsertSound( SOUND_COMBAT, GetAbsOrigin(), 512, 3.0, this, SOUNDENT_CHANNEL_WEAPON );
...
int CObjectSentrygun::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	{
		m_flRecoveryTime = gpGlobals->curtime + 4.0f;
		m_iState.Set( SENTRY_STATE_SAPPER_RECOVERY );
	}
...
void CObjectSentrygun::Killed( const CTakeDamageInfo &info )
{
	...
	if ( pOwner )
	{
		CTFLaserPointer *pWrangler = dynamic_cast <CTFLaserPointer *> ( pOwner->Weapon_GetSlot( LOADOUT_POSITION_SECONDARY ) );
		if ( pWrangler )
		{
		...
		pOwner->m_Shared.SetRevengeCrits( GetAssists() + ( GetKills() * 2 ) );
	}
...
void CObjectSentrygun::EmitSentrySound( IRecipientFilter &filter, int index, char const* pszSound  )
{
	...
	CSoundEnt::InsertSound( SOUND_MOVE_AWAY, GetAbsOrigin(), 256, 3.0, this, SOUNDENT_CHANNEL_REPEATED_DANGER );
