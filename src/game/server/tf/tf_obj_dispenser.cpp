...
#define DISPENSER_TRIGGER_MINS			Vector( -70, -70, -70 )
#define DISPENSER_TRIGGER_MAXS			Vector( 70, 70, 70 )	// tweak me
...
IMPLEMENT_SERVERCLASS_ST( CObjectDispenser, DT_ObjectDispenser )
	...
	SendPropString( SENDINFO( m_szMaterialOverride ) ),
	SendPropArray2( 
...
ConVar lfe_obj_dispenser_screen( "lfe_obj_dispenser_screen", "0", FCVAR_CHEAT );
...
IMPLEMENT_SERVERCLASS_ST( CDispenserTouchTrigger, DT_DispenserTouchTrigger )
END_SEND_TABLE()
	...
	*/
	...
	else
	{
		...
		if ( pTriggerEnt )
		{
			...
			UTIL_SetSize( pTriggerEnt, DISPENSER_TRIGGER_MINS, DISPENSER_TRIGGER_MAXS );
			pTriggerEnt->SetParent( this );
			m_hTouchTrigger = pTriggerEnt;
		}
	}
...
void CObjectDispenser::GetControlPanelInfo( int nPanelIndex, const char *&pPanelName )
{
	...
	{
		if ( lfe_obj_dispenser_screen.GetBool() )
		{
...
bool CObjectDispenser::DispenseAmmo( CTFPlayer *pPlayer )
{
	...
	// grenade
	int iGrenade1 = pPlayer->GiveAmmo( floor( pPlayer->GetMaxAmmo( LFE_AMMO_GRENADES1 ) * flAmmoRate ), LFE_AMMO_GRENADES1, false, TF_AMMO_SOURCE_DISPENSER );
	iTotalPickedUp += iGrenade1;

	int iGrenade2 = pPlayer->GiveAmmo( floor( pPlayer->GetMaxAmmo( LFE_AMMO_GRENADES2 ) * flAmmoRate ), LFE_AMMO_GRENADES2, false, TF_AMMO_SOURCE_DISPENSER );
	iTotalPickedUp += iGrenade2;
...
void CObjectDispenser::StartHealing( CBaseEntity *pOther )
{
	...
	if ( pPlayer )
	{
		pPlayer->m_Shared.Heal( GetBuilder(), GetHealRate(), true );
	}
	else if ( pOther && pOther->IsNPC() )
	{
		CAI_BaseNPC *pNPC = assert_cast<CAI_BaseNPC *>( pOther );
		pNPC->Heal( GetBuilder(), GetHealRate(), true );
	}
}
...
void CObjectDispenser::StopHealing( CBaseEntity *pOther )
{
	...
	if ( bFound )
	{
		...
		if ( pPlayer )
		{
			pPlayer->m_Shared.StopHealing( GetBuilder() );
		}
		else if ( pOther && pOther->IsNPC() )
		{	
			pNPC->StopHealing( GetBuilder() );
		}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CObjectDispenser::SetCustomHTML( const char *pszHTML )
{
	Q_snprintf( m_szCustomHTML.GetForModify(), MAX_PATH, pszHTML );
}
