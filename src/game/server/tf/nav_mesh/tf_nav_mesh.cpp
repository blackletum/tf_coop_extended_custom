...
{
	CTraceFilterSimple filter( pBlocker, COLLISION_GROUP_NONE );
...
void CTFNavMesh::ComputeBlockedAreas()
{
	...
	for ( CBaseEntity *pDoor = gEntList.FirstEnt(); pDoor; pDoor = gEntList.FindEntityByClassname( pDoor, "func_door*" ) )
	{
		...
		bool bDoorClosed = ( (CBaseToggle *)pDoor )->m_toggle_state == TS_AT_BOTTOM || ( (CBaseToggle *)pDoor )->m_toggle_state == TS_GOING_DOWN;
		...
		{
			CBaseTrigger *pTrigger = dynamic_cast<CBaseTrigger *>( pEntity );
			if ( pTrigger == nullptr )
				break;
		...
		for ( int i=0; i<potentiallyBlockedAreas.Count(); ++i )
		{
			...
			if ( ( !bNoFilter && bDoorClosed ) || iNavTeam > 0 )
				continue;
...
public:
	...
	virtual bool ShouldSearch( CNavArea *adjArea, CNavArea *currentArea, float travelDistanceSoFar )
	{
		...
		if ( !TFGameRules()->IsHordeMode() && !adjTFArea->HasTFAttributes( RED_SETUP_GATE|BLUE_SETUP_GATE|SPAWN_ROOM_EXIT ) && adjArea->IsBlocked( m_iTeam ) )
			return false;*/
...
void CTFNavMesh::DecorateMesh()
{
	...
	for ( CBaseEntity *pEnt = gEntList.FirstEnt(); pEnt; pEnt = gEntList.NextEnt( pEnt ) )
	{
		...
		if ( area )
		{
			...
			else if ( FClassnameIs( pEnt, "bot_hint_sentrygun" ) )
				area->AddTFAttributes( SENTRY_SPOT );
		}
	}
