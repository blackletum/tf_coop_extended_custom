...
ConVar lfe_obj_teleporter_twoways( "lfe_obj_teleporter_twoways", "1", FCVAR_NOTIFY | FCVAR_REPLICATED, "Enable MvM 2-way teleporters." );
...
void CObjectTeleporter::TeleporterTouch( CBaseEntity *pOther )
{
	...
	CALL_ATTRIB_HOOK_INT_ON_OTHER( GetBuilder(), bTwoWayTeleporter, bidirectional_teleport );

	// is this an entrance and do we have an exit?
	if ( GetObjectMode() == TELEPORTER_TYPE_ENTRANCE || ( ( bTwoWayTeleporter != 0 || lfe_obj_teleporter_twoways.GetBool() ) && GetObjectMode() == TELEPORTER_TYPE_EXIT ) )
	{		
