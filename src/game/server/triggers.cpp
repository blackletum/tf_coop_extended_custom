...
bool CBaseTrigger::PassesTriggerFilters(CBaseEntity *pOther)
{
	...
	if ( HasSpawnFlags(SF_TRIGGER_ALLOW_ALL) ||
		...
		(HasSpawnFlags(SF_TRIG_TOUCH_DEBRIS) && (pOther->GetCollisionGroup() == COLLISION_GROUP_DEBRIS ||
...
BEGIN_DATADESC( CTriggerHurt )
	...
	DEFINE_KEYFIELD( m_iCustomDamageType, FIELD_INTEGER, "damagecustomtype" ),
...
bool CTriggerHurt::HurtEntity( CBaseEntity *pOther, float damage )
{
	...
	else
	{
		...
		CTakeDamageInfo info( this, this, damage, m_bitsDamageInflict, m_iCustomDamageType );
...
private:
	...
	void ChangeLevelNow( CBaseEntity *pActivator, bool bForce );
...
void CChangeLevel::InputChangeLevel( inputdata_t &inputdata )
{
	...
	ChangeLevelNow( inputdata.pActivator, true );
}
...
extern ConVar lfe_transition_players_percent;

void CChangeLevel::ChangeLevelNow( CBaseEntity *pActivator, bool bForce )
{
	CTFPlayer *pPlayer = ToTFPlayer( pActivator );
	if ( !pPlayer )
		return;

	if ( !pPlayer->m_bInTransition && TFGameRules()->IsAnyCoOp() && pPlayer->IsOnStoryTeam() && pPlayer->IsAlive() )
	{
		TFGameRules()->BroadcastSound( 255, "Hud.EndRoundScored" );
		pPlayer->m_bInTransition = true;
	...
	if ( lfe_transition_players_percent.GetInt() > 0 && !bForce )
	{
		CTeam *pRedTeam = GetGlobalTeam( TF_STORY_TEAM );
		Assert( pRedTeam );

		int totalPlayers = pRedTeam->GetNumPlayers();
		int transitionPlayers = 0;
		for ( int i = 0; i < totalPlayers; i++ )
		{
			CTFPlayer *pAllPlayer = ToTFPlayer( pRedTeam->GetPlayer( i ) );
			if ( pAllPlayer && pAllPlayer->m_bInTransition )
				transitionPlayers++;
		}

		float flMaxPercent = roundf( (float)transitionPlayers / (float)totalPlayers * 100 );
		int nMinPercent = lfe_transition_players_percent.GetInt();
		if ( flMaxPercent < nMinPercent && !bForce )
		{
			ConDColorMsg( Color( 77, 116, 85, 255 ), "[TFGameRules] Level Transition Percent: %f/%i\n", flMaxPercent, nMinPercent );
			return;
		}
	}
	...
	{
		if ( g_voteController )
			g_voteController->CreateVote( pPlayer->entindex(), "changelevel", st_szNextMap );
	}
	else
	{
		TFGameRules()->ChangeLevel( st_szNextMap, true, true, true );
	}
}
...
void CTriggerCamera::Disable( void )
{
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		if ( m_hPlayer && m_hPlayer->IsAlive() )
		{
			...
			if ( !m_bDontSetPlayerView )
				pBasePlayer->SetViewEntity( m_hPlayer );
			...
			{
				pBasePlayer->GetActiveWeapon()->RemoveEffects( EF_NODRAW );
			}
		}
