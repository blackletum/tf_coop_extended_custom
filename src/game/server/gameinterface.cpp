...
//Do we even have a lfe preprocessor?
#ifdef TF_CLASSIC
...
{
#ifdef TF_CLASSIC
	// Calling as soon as DLLShutdown.
...
void CServerGameDLL::CreateNetworkStringTables( void )
{
	...
	g_pStringTableServerMapAddFiles = networkstringtable->CreateStringTable( "ServerMapAddFiles", 128 );
	...
	Assert( g_pStringTableParticleEffectNames &&
			...
			bPopFilesValid &&
			g_pStringTableServerMapAddFiles
			);
                                            ...
                                            /* in/out */ char *pszMapFile, size_t nMapFileSize )
{
	...
	AddRequiredMapSearchPaths( pszMapName );
}
...
void CServerGameClients::ClientSetupVisibility( edict_t *pViewEntity, edict_t *pClient, unsigned char *pvs, int pvssize )
{
	...
	if ( pPlayer )
	{
		...
		CTFPlayer* pPortalPlayer = static_cast<CTFPlayer*>( pPlayer );
		if ( pPortalPlayer )
		{
