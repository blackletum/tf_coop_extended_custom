...
BEGIN_DATADESC( CPropVehicleDriveable )
	...
	DEFINE_INPUTFUNC( FIELD_INTEGER, "EnterVehicleRole", InputEnterVehicleRole ),
...
void CPropVehicleDriveable::DriveVehicle( float flFrameTime, CUserCmd *ucmd, int iButtonsDown, int iButtonsReleased )
{
	...
	if ( iButtonsDown & IN_ATTACK3 )
	{
		m_pressedAttack3.FireOutput( this, this, 0 );
	}
	...
	{
		m_pressedMoveRight.FireOutput( this, this, 0 );
	}
	if ( iButtonsDown & IN_MOVELEFT )
	{
		m_pressedMoveLeft.FireOutput( this, this, 0 );
	}
	if ( iButtonsDown & IN_FORWARD )
	{
		m_pressedForward.FireOutput( this, this, 0 );
	}
	if ( iButtonsDown & IN_BACK )
	{
		m_pressedBack.FireOutput( this, this, 0 );
	}
	if ( iButtonsDown & IN_JUMP )
	{
		m_pressedJump.FireOutput( this, this, 0 );
	}
	if ( iButtonsDown & IN_DUCK )
	{
		m_pressedCrouch.FireOutput( this, this, 0 );
	}
	if ( iButtonsDown & IN_SPEED )
	{
		m_pressedSprint.FireOutput( this, this, 0 );
	}
	if ( iButtonsDown & IN_RELOAD )
	{
		m_pressedReload.FireOutput( this, this, 0 );
	}
}
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CPropVehicleDriveable::InputEnterVehicleRole( inputdata_t &inputdata )
{
	if ( m_bEnterAnimOn )
		return;
	...
	if ( pPlayer != NULL )
	{
		...
		if ( inputdata.value.Int() > VEHICLE_SEAT_INVALID )
			pPlayer->GetInVehicle( GetServerVehicle(), inputdata.value.Int() );
		else
			pPlayer->GetInVehicle( GetServerVehicle(), VEHICLE_ROLE_DRIVER );
	}
	else
...
// Purpose: 
//-----------------------------------------------------------------------------
int CPropVehicleDriveable::GetEyesAttachment( void )
{ 
	return LookupAttachment( "vehicle_driver_eyes" );
}
