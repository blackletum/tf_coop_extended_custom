...
CON_COMMAND_F( ai_set_move_height_epsilon, "Set how high AI bumps up ground walkers when checking steps", FCVAR_CHEAT )
{
	if ( args.ArgC() > 1 )
	{
...
bool CAI_MoveProbe::ShouldBrushBeIgnored( CBaseEntity *pEntity )
{
	...
	if (pEntity->ClassMatches("func_antirush"))
	{
		return true;
	}
	...
	const Vector &hullMax, unsigned int mask, trace_t *pResult ) const
{
	...
	if ( !m_pTraceListData || m_pTraceListData->IsEmpty() )
	{
		...
		CProp_Portal* pPortal = UTIL_Portal_FirstAlongRay( ray, flMustBeCloserThan );
		UTIL_Portal_TraceRay_With( pPortal, ray, mask, &traceFilter, pResult );
...
bool CAI_MoveProbe::CheckStep( const CheckStepArgs_t &args, CheckStepResult_t *pResult ) const
{
	...
	if ( trace.startsolid || ( trace.fraction < 1.0f ) )
	{
		...
		CTraceFilterNav traceFilter( const_cast<CAI_BaseNPC *>(GetOuter()), m_bIgnoreTransientEntities, GetOuter(), collisionMask );
		Ray_t ray;
		ray.Init( stepStart, stepEnd, WorldAlignMins(), WorldAlignMaxs() );
		if ( trace.fraction == 1.0f && UTIL_DidTraceTouchPortals( ray, trace ) )
		{
			trace_t tempTrace;
			UTIL_Portal_TraceEntity( const_cast<CAI_BaseNPC *>(GetOuter()), stepStart, stepEnd, collisionMask, &traceFilter, &tempTrace );

			if ( tempTrace.DidHit() && tempTrace.fraction < trace.fraction && !tempTrace.startsolid && !tempTrace.allsolid )
			{
				pResult->hitNormal = tempTrace.plane.normal;
			}
		}
	...
	unsigned int collisionMask, const CBaseEntity *pTarget, AIMoveTrace_t *pMoveTrace ) const
{
	...
	float flNPCMinJumpHeight = 0; //GetOuter()->GetMinJumpHeight();
