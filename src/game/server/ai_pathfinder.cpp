...
public:
 	...
 	float operator() (CNavArea *area, CNavArea *fromArea, const CNavLadder *ladder, const CFuncElevator *elevator, float length)
	{
		...
		if ( !m_bot->IsAreaTraversable( area ) )
		{
			// dead end
			return -1.0f;
		}
		...
		else*/
		{
			...
			const int iOtherTeam = GetEnemyTeam( m_bot );
				...
				if ( obj->GetType() == OBJ_SENTRYGUN && obj->GetTeamNumber() == iOtherTeam )
				{
					...
					if ( area == obj->GetLastKnownArea() )
					{
						if ( m_route == SAFEST_ROUTE )
							dist *= 5.0f;
						else if ( m_bot->Classify() == CLASS_COMBINE )
							dist *= 10.0f;
					}
				}
			}
			...
			if ( area->HasAttributes( NAV_MESH_FUNC_COST ) )
				cost *= area->ComputeFuncNavCost( m_bot );
			...
			if ( !area->IsUnderwater() && area->IsConnected( fromArea, NUM_DIRECTIONS ) == false )
			{
				...
				float fallDistance = -fromArea->ComputeAdjacentConnectionHeightChange( area ); //float fallDistance = -fromArea->ComputeGroundHeightChange( area );
...
AI_Waypoint_t *CAI_Pathfinder::BuildRoute( const Vector &vStart, const Vector &vEnd, CBaseEntity *pTarget, float goalTolerance, Navigation_t curNavType, bool bLocalSucceedOnWithinTolerance )
{
	...
	{
		int nLocalBuildFlags = buildFlags;
...
public:
		...
		m_pRoute( NULL )
	{
		...
		COMPILE_TIME_ASSERT( bits_BUILD_GROUND == (int)bits_CAP_MOVE_GROUND && bits_BUILD_FLY == (int)bits_CAP_MOVE_FLY && bits_BUILD_JUMP == (int)bits_CAP_MOVE_JUMP && bits_BUILD_CLIMB == (int)bits_CAP_MOVE_CLIMB && bits_BUILD_CRAWL == (int)bits_CAP_MOVE_CRAWL );
	}
