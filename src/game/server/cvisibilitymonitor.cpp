...
void CVisibilityMonitor::AddEntity( CBaseEntity *pEntity, float flMinDist, VisibilityMonitorCallback pfnCallback, VisibilityMonitorEvaluator pfnEvaluator )
{
	...
	if( !IsTrackingEntity( pEntity, pfnCallback, pfnEvaluator ) )
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CVisibilityMonitor::IsTrackingEntity( CBaseEntity *pEntity, VisibilityMonitorCallback pfnCallback, VisibilityMonitorEvaluator pfnEvaluator)
{
	Assert( pEntity != NULL );

	for( int i = 0 ; i < m_Entities.Count() ; i++ )
	{
		if( m_Entities[i].entity == pEntity && m_Entities[i].pfnEvaluator == pfnEvaluator)
		{
			return true;
		}
	}
