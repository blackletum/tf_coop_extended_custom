...
void CNPC_Portal_FloorTurret::ActiveThink( void )
{
	...
	if ( ( bEnemyVisible == false ) || ( m_flDistToEnemy > PORTAL_FLOOR_TURRET_RANGE ) || ( pTFEnemy && ( pTFEnemy->m_Shared.GetPercentInvisible() > 0.5 || ( pTFEnemy->m_Shared.InCond( TF_COND_DISGUISED ) && pTFEnemy->m_Shared.GetDisguiseTeam() == GetTeamNumber() ) ) ) )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Portal_FloorTurret::OnAttachSapper( void )
{
	SelfDestruct();
}