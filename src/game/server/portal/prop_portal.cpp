...
const Vector &CProp_Portal::GetPortalLocalMins() const
{
	static Vector mins(0.0f, 0.0f, 0.0f);
	mins.y = -m_flWidth;
	mins.z = -m_flHeight;
	return mins;
}

const Vector &CProp_Portal::GetPortalLocalMaxs() const
{
	static Vector maxs(64.0f, 0.0f, 0.0f);
	maxs.y = m_flWidth;
	maxs.z = m_flHeight;
	return maxs;
}
...
CProp_Portal::CProp_Portal( void )
{
	...
	m_flWidth = PORTAL_HALF_WIDTH;
	m_flHeight = PORTAL_HALF_HEIGHT;
	...
	fPlanes[(0*4) + 1] = 0.0f;
	fPlanes[(0*4) + 2] = 0.0f;
	fPlanes[(0*4) + 3] = GetPortalLocalMaxs().x;

	fPlanes[(1*4) + 0] = -1.0f;
	fPlanes[(1*4) + 1] = 0.0f;
	fPlanes[(1*4) + 2] = 0.0f;
	fPlanes[(1*4) + 3] = -GetPortalLocalMins().x;

	fPlanes[(2*4) + 0] = 0.0f;
	fPlanes[(2*4) + 1] = 1.0f;
	fPlanes[(2*4) + 2] = 0.0f;
	fPlanes[(2*4) + 3] = GetPortalLocalMaxs().y;

	fPlanes[(3*4) + 0] = 0.0f;
	fPlanes[(3*4) + 1] = -1.0f;
	fPlanes[(3*4) + 2] = 0.0f;
	fPlanes[(3*4) + 3] = -GetPortalLocalMins().y;

	fPlanes[(4*4) + 0] = 0.0f;
	fPlanes[(4*4) + 1] = 0.0f;
	fPlanes[(4*4) + 2] = 1.0f;
	fPlanes[(4*4) + 3] = GetPortalLocalMaxs().z;

	fPlanes[(5*4) + 0] = 0.0f;
	fPlanes[(5*4) + 1] = 0.0f;
	fPlanes[(5*4) + 2] = -1.0f;
	fPlanes[(5*4) + 3] = -GetPortalLocalMins().z;
...
void CProp_Portal::DelayedPlacementThink( void )
{
	...
	if( pPortalGun )
	{
		CTFPlayer *pFiringPlayer = dynamic_cast<CTFPlayer *>( pPortalGun->GetOwner() );
		if( pFiringPlayer )
		{
...
void CProp_Portal::TestRestingSurfaceThink( void )
{
	...
	for ( int iCorner = 0; iCorner < 4; ++iCorner )
	{
		...
		if ( iCorner % 2 == 0 )
			vCorner += vRight * ( m_flWidth - PORTAL_BUMP_FORGIVENESS * 1.1f );
		else
			vCorner += -vRight * ( m_flWidth - PORTAL_BUMP_FORGIVENESS * 1.1f );

		if ( iCorner < 2 )
			vCorner += vUp * ( m_flHeight - PORTAL_BUMP_FORGIVENESS * 1.1f );
		else
			vCorner += -vUp * ( m_flHeight - PORTAL_BUMP_FORGIVENESS * 1.1f );
...
void CProp_Portal::DoFizzleEffect( int iEffect, bool bDelayedPos /*= true*/ )
{
	...
	switch ( iEffect )
	{
		...
		{
			if ( !m_bIsPortal2 && m_hLinkedPortal.m_Value.IsValid() )
			{
				Vector vLinkedForward;
...
void CProp_Portal::TeleportTouchingEntity( CBaseEntity *pOther )
{
	...
	bool bNPC	 = pOther->IsNPC();
	QAngle qPlayerEyeAngles;
	CTFPlayer *pOtherAsPlayer;

	
	if( bPlayer )
	{
		//NDebugOverlay::EntityBounds( pOther, 255, 0, 0, 128, 60.0f );
		pOtherAsPlayer = (CTFPlayer *)pOther;
	...
	//apply transforms to relevant variables (applied to the entity later)
	{
		...
		UTIL_Portal_VectorTransform( m_matrixThisToLinked, vOtherVelocity, vNewVelocity );
	}
	...
	//do the actual teleportation
	{
		...
		else if ( bNPC && pOther->GetMoveType() == MOVETYPE_STEP )
		{
			qNewAngles[PITCH] = 0;
			qNewAngles[ROLL] = 0;
	...
	{
		CTFPlayer *pHoldingPlayer = (CTFPlayer *)GetPlayerHoldingEntity( pOther );
...
void CProp_Portal::Touch( CBaseEntity *pOther )
{
	...
	if( !m_bActivated || (m_hLinkedPortal.Get() == NULL) )
	{
		...
		Assert( !pOther->IsPlayer() || (((CTFPlayer *)pOther)->m_hPortalEnvironment.Get() != this) );
...
void CProp_Portal::EndTouch( CBaseEntity *pOther )
{
	...
	if( ShouldTeleportTouchingEntity( pOther ) ) //an object passed through the plane and all the way out of the touch box
		TeleportTouchingEntity( pOther );
			...
			(((CTFPlayer *)pOther)->m_Local.m_bInDuckJump) ) //while ducking
	{
...
{
	if( pEntity->IsPlayer() )
		((CTFPlayer *)pEntity)->m_hPortalEnvironment = this;
}

void CProp_Portal::PortalSimulator_ReleasedOwnershipOfEntity( CBaseEntity *pEntity )
{
	if( pEntity->IsPlayer() && (((CTFPlayer *)pEntity)->m_hPortalEnvironment.Get() == this) )
		((CTFPlayer *)pEntity)->m_hPortalEnvironment = NULL;
}
...
void CProp_Portal::WakeNearbyEntities( void )
{
	...
	ptOBBStart += vForward * GetPortalLocalMins().x;
	ptOBBStart += vRight * GetPortalLocalMins().y;
	ptOBBStart += vUp * GetPortalLocalMins().z;


	vForward *= GetPortalLocalMaxs().x - GetPortalLocalMins().x;
	vRight *= GetPortalLocalMaxs().y - GetPortalLocalMins().y;
	vUp *= GetPortalLocalMaxs().z - GetPortalLocalMins().z;
	...
	for ( int i = 0; i < count; i++ )
	{
		...
		if ( pEntity && (pEntity != this) )
		{
			...
			if( IsOBBIntersectingOBB( ptOrigin, qAngles, GetPortalLocalMins(), GetPortalLocalMaxs(), 
...
void CProp_Portal::PlacePortal( const Vector &vOrigin, const QAngle &qAngles, float fPlacementSuccess, bool bDelay /*= false*/ )
{
	...
	if ( fPlacementSuccess < 0.5f )
	{
		...
		if( pPortalGun )
		{
			...
			if( pFiringPlayer )
			{
				ChangeTeam( pFiringPlayer->GetTeamNumber() );
...
void CProp_Portal::UpdateCorners()
{
	...
	for ( int i = 0; i < 4; ++i )
	{
		...
		vAddPoint += vRight * ((i & (1<<0))?(m_flWidth):(-m_flWidth));
		vAddPoint += vUp * ((i & (1<<1))?(m_flHeight):(-m_flHeight));
...
bool CProp_Portal::ShouldCollide( int collisionGroup, int contentsMask ) const
{
	return false;
}
