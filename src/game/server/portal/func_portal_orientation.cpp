...
bool UTIL_TestForOrientationVolumes( QAngle& vecCurAngles, const Vector& vecCurOrigin, const CProp_Portal* pPortal )
{
	...
	Vector vPortalOBBMin = pPortal->GetPortalLocalMins();
	Vector vPortalOBBMax = pPortal->GetPortalLocalMaxs();
	...
	while ( pList )
	{
		...
		if ( IsOBBIntersectingOBB( vecCurOrigin, vecCurAngles, vPortalOBBMin, vPortalOBBMax,
...
void CFuncPortalOrientation::OnActivate( void )
{
	...
	if( iPortalCount != 0 )
	{
		...
		for( int i = 0; i != iPortalCount; ++i )
		{
			...
			Vector vPortalOBBMin = pTempPortal->GetPortalLocalMins();
			Vector vPortalOBBMax = pTempPortal->GetPortalLocalMaxs();

			if( IsOBBIntersectingOBB( pTempPortal->GetAbsOrigin(), pTempPortal->GetAbsAngles(), vPortalOBBMin, vPortalOBBMax,
