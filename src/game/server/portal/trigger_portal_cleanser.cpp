...
void CTriggerPortalCleanser::Touch( CBaseEntity *pOther )
{
	...
	if ( pOther->IsPlayer() )
	{
		...
		if ( pPlayer )
		{
			CWeaponPortalgun *pPortalgun = dynamic_cast<CWeaponPortalgun*>( pPlayer->Weapon_OwnsThisType( "tf_weapon_portalgun" ) );
	...
	if ( pBaseAnimating && !pBaseAnimating->IsDissolving() )
	{
		...
		{
			CTFPlayer *pPlayer = (CTFPlayer *)GetPlayerHoldingEntity( pBaseAnimating );
			if( pPlayer )
			{
