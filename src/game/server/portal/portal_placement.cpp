...
bool IsPortalOverlappingOtherPortals( const CProp_Portal *pIgnorePortal, const Vector &vOrigin, const QAngle &qAngles, bool bFizzle /*= false*/ )
{
	...
	//Vector vPortalOBBMin = pIgnorePortal->GetPortalLocalMins() + Vector( 1.0f, 1.0f, 1.0f );
	//Vector vPortalOBBMax = pIgnorePortal->GetPortalLocalMaxs() - Vector( 1.0f, 1.0f, 1.0f );
	...
	if( iPortalCount != 0 )
	{
		...
		for( int i = 0; i != iPortalCount; ++i )
		{
			...
			if( pTempPortal != pIgnorePortal && pTempPortal->m_bActivated )
			{
				...
				Vector vOtherPortalOBBMin = pTempPortal->GetPortalLocalMins() + Vector(1.0f, 1.0f, 1.0f);
				Vector vOtherPortalOBBMax = pTempPortal->GetPortalLocalMaxs() - Vector(1.0f, 1.0f, 1.0f);

				if ( IsOBBIntersectingOBB( vOrigin, qAngles, vOtherPortalOBBMin, vOtherPortalOBBMax,
										   vOtherOrigin, qOtherAngles, vOtherPortalOBBMin, vOtherPortalOBBMax, 0.0f ) )
				{
					if ( sv_portal_placement_debug.GetBool() )
					{
						UTIL_Portal_NDebugOverlay( vOrigin, qAngles, 0, 0, 255, 128, false, 0.5f );
