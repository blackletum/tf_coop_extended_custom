		...
		for ( int iTeam = FIRST_GAME_TEAM; iTeam < nTeamCount; ++iTeam )
		{
			...
			if ( pTeam )
			{
				...
				for ( int iObject = 0; iObject < pTeam->GetNumObjects(); iObject++ )
				{
					...
					if ( pObject )
					{
						if ( origin.DistToSqr(pObject->GetAbsOrigin()) < distSq && Look( pObject ) )
						{
							nSeen++;
						}
						else
						{
							CProp_Portal *pPortal = GetOuter()->FInViewConeThroughPortal( pObject );
							if ( pPortal && UTIL_Portal_DistanceThroughPortalSqr( pPortal, origin, pObject->GetAbsOrigin() ) < distSq && LookThroughPortal( pPortal, pObject ) )
							{
								nSeen++;
							}
						}
...
{
	if ( ( pEntity->GetFlags() & FL_OBJECT ) && !pEntity->IsPlayer() && !pEntity->IsNPC() && !pEntity->IsBaseObject() )
	{
...
void CAI_SensedObjectsManager::AddEntity( CBaseEntity *pEntity )
{
	...
	Assert( !pEntity->IsPlayer() && !pEntity->IsNPC() && !pEntity->IsBaseObject() );
