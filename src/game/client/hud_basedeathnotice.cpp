...
void CHudBaseDeathNotice::Init( void )
{
	...
	ListenForGameEvent( "lfe_deathnotice_text" );
}
...
void CHudBaseDeathNotice::FireGameEvent( IGameEvent *event )
{
	...
	bool bFishDeath = FStrEq( pszEventName, "fish_notice" );
	...
	int iMsg = -1;
	if ( bPlayerDeath || bNPCDeath || bFishDeath )
	{
	...
	if ( bPlayerDeath || bObjectDeath || bNPCDeath || bFishDeath )
	{
		...
		if ( !killer || killer == victim )
		{
			...
			if ( bFishDeath )
			{
				if ( bPlayerDeath || bNPCDeath )
				{
					/*iHitCount++;
					char sFishMsg[64];
					Q_snprintf( sFishMsg, sizeof( sFishMsg ), g_pVGuiLocalize->Find( "#Humiliation_Count" ), iHitCount );*/
					V_wcsncpy( m_DeathNotices[iMsg].wzInfoText, g_pVGuiLocalize->Find( "#Humiliation_Count" ), sizeof( m_DeathNotices[iMsg].wzInfoText ) );
				}
				else
				{
					V_wcsncpy( m_DeathNotices[iMsg].wzInfoText, g_pVGuiLocalize->Find( "#Humiliation_Kill" ), sizeof( m_DeathNotices[iMsg].wzInfoText ) );
				}
			}
		}
	...
	else if ( FStrEq( "teamplay_capture_blocked", pszEventName ) )
	{
		...
		const char *szBlockerClassname = event->GetString( "blocker_name" );
		const char *szBlockerName = NULL;
		if ( IsPlayerIndex( iPlayerIndex ) )
		{
			szBlockerName = g_PR->GetPlayerName( iPlayerIndex );
		}
		else
		{
			char nameBuf[MAX_PLAYER_NAME_LENGTH * 2];
			GetLocalizedNPCName( szBlockerClassname, nameBuf, sizeof( nameBuf ) );
			szBlockerName = nameBuf;
		}

		Q_strncpy( m_DeathNotices[iMsg].Killer.szName, szBlockerName, ARRAYSIZE( m_DeathNotices[iMsg].Killer.szName ) );
		m_DeathNotices[iMsg].Killer.iTeam = g_PR->GetTeam( iPlayerIndex );
		if ( iLocalPlayerIndex == iPlayerIndex )
			m_DeathNotices[iMsg].bLocalPlayerInvolved = true;
	...
	else if ( FStrEq( "teamplay_flag_event", pszEventName ) )
	{
		...
		bool bIsHorde = TFGameRules() && TFGameRules()->IsHordeMode();
		if ( bIsHorde )
		{
			// Horde only cares about Defend notifications
			if ( iEventType != TF_FLAGEVENT_DEFEND )
			{
				// unsupported, don't put anything up			
				m_DeathNotices.Remove( iMsg );
		...
		case TF_FLAGEVENT_DEFEND: 
			...
			else if ( bIsHalloween2014 )
			{
				pszMsgKey = "#Msg_DefendedFlagHalloween2014";
			}
			else if ( bIsHorde )
			{
				pszMsgKey = "#Msg_DefendedBomb";
		...
		const char *szPlayerClassname = event->GetString( "player_name" );
		const char *szPlayerName = NULL;
		if ( IsPlayerIndex( iPlayerIndex ) )
		{
			szPlayerName = g_PR->GetPlayerName( iPlayerIndex );
		}
		else
		{
			char nameBuf[MAX_PLAYER_NAME_LENGTH * 2];
			GetLocalizedNPCName( szPlayerClassname, nameBuf, sizeof( nameBuf ) );
			szPlayerName = nameBuf;
		}
	...
	else if ( FStrEq( "lfe_deathnotice_text", pszEventName ) )
	{
		...
		m_DeathNotices[iMsg].bCrit = bCrit;
	}
