...
void CPDAPanel_Spy_Invis_Pocket::OnTick( void )
{
	...
	if ( pPlayer && !pPlayer->IsDormant() )
	{
		if ( m_pInvisProgress )
		{
			//m_pInvisProgress->SetSize( m_pInvisProgress->GetWide() + 9, m_pInvisProgress->GetTall() + 18 );
			//m_pInvisProgress->SetPos( m_pInvisProgress->GetXPos() + 20, m_pInvisProgress->GetYPos() + -15 );
			m_pInvisProgress->SetProgress( pPlayer->m_Shared.GetSpyCloakMeter() / 100.0f );
		}
	}	
}
...
//-----------------------------------------------------------------------------
// Spy Enthusiast's Timepiece PDA
//-----------------------------------------------------------------------------
...
DECLARE_VGUI_SCREEN_FACTORY( CPDAPanel_Spy_Invis_Pocket_HM, "pda_spy_invis_pocket_hm" );
