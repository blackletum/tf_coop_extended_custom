...
CON_COMMAND( showmapinfo, "Show map info panel" )
{
	...
	if ( pPlayer )
	{
		     ...
		     ( ( pPlayer->GetTeamNumber() != TEAM_UNASSIGNED ) && ( pPlayer->GetPlayerClass()->GetClassIndex() != TF_CLASS_UNDEFINED ) ) )
		{
			...
			gViewPortInterface->ShowPanel( PANEL_COOPTEAMSELECT, false );
...
//		newpanel = new CTFMapOverview( this );
	...
	else if ( Q_strcmp( PANEL_COOPTEAMSELECT, szPanelName ) == 0)
	{
		newpanel = new CTFCoOpTeamMenu( this );
	}
...
void TFViewport::CreateDefaultPanels( void )
{
	...
	AddNewPanel( CreatePanelByName( PANEL_COOPTEAMSELECT ), "PANEL_COOPTEAMSELECT" );
