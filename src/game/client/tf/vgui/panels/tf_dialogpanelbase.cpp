...
void CTFDialogPanelBase::Show()
{
	...
	int _x, _y;
	GetPos(_x, _y);
	SetPos(_x, _y - YRES(25));
	AnimationController::PublicValue_t p_AnimHover(_x, _y);
	vgui::GetAnimationController()->RunAnimationCommand(this, "Position", p_AnimHover, 0.0f, 0.2f, vgui::AnimationController::INTERPOLATOR_SIMPLESPLINE, NULL);
	MAINMENU_ROOT->ShowPanel(SHADEBACKGROUND_MENU);
	//GetMenuPanel(DASHBOARD_MENU)->Show();
};
...
void CTFDialogPanelBase::OnKeyCodeTyped( vgui::KeyCode code )
{
