...
void CTFSpectatorGUI::UpdateReinforcements( void )
{
	...
	{
		g_pVGuiLocalize->ConstructString( wLabel, sizeof( wLabel ), g_pVGuiLocalize->Find( TFGameRules()->CanPlayerStopSearchSpawn() ? "#game_respawntime_stopsearch"  : "#game_respawntime_searching" ), 0 );
	}
	else
	{
