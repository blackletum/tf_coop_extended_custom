...
#ifndef __VCTFGenericPanelList_H__
#define __VCTFGenericPanelList_H__

#include "panels/tf_menupanelbase.h"

class CTFGenericPanelList;
...
class CTFGenericPanelList : public vgui::Panel
{
	DECLARE_CLASS_SIMPLE( CTFGenericPanelList, vgui::Panel );

public:
	enum ITEM_SELECTION_MODE { ISM_PERITEM = 1, ISM_ELEVATOR = 2, ISM_ALPHA_INVISIBLE = 4 };
	CTFGenericPanelList(vgui::Panel *parent, const char *panelName, int selectionModeMask);
	virtual ~CTFGenericPanelList();
...
protected:	
	...
	void SetNavigationChangedCallback( void (*pFunction)( CTFGenericPanelList *, vgui::Panel * ) );
...
private:
	...
	bool m_bWrap;
	void (*m_pItemNavigationChangedCallback)( CTFGenericPanelList *pThis, vgui::Panel *pPanel );
};

class ITFGenericPanelListItem
{
public:
	virtual bool IsLabel() = 0;
};

#endif // __VCTFGenericPanelList_H__