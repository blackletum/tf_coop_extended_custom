...
CTFButtonBase::CTFButtonBase( Panel *parent, const char *panelName, const char *text ) : Button( parent, panelName, text )
{
	...
	//m_pButtonRotatingImage = new CTFRotatingImagePanel( this, "SubRotatingImage" );
...
void CTFButtonBase::ApplySchemeSettings( IScheme *pScheme )
{
	...
	//m_pButtonRotatingImage->SetMouseInputEnabled( false );
...
void CTFButtonBase::ApplySettings( KeyValues *inResourceData )
{
	...
	/*KeyValues *pRotatingImageKey = inResourceData->FindKey( "SubRotatingImage" );
	if ( pRotatingImageKey )
	{
		m_pButtonRotatingImage = new CTFRotatingImagePanel( this, "SubRotatingImage" );

		// Workaround for this not being an EditablePanel.
		if ( m_pButtonRotatingImage )
			m_pButtonRotatingImage->ApplySettings( pRotatingImageKey );

		m_iImageWidth = pRotatingImageKey->GetInt( "imagewidth", 0 );
	}*/
...
void CTFButtonBase::SetImage( const char *pszImage )
{
	...
	if ( m_pButtonRotatingImage )
		m_pButtonRotatingImage->SetImage( pszImage );
}
...
void CTFButtonBase::SetImageInset( int iInsetX, int iInsetY )
{
	...
	if ( m_pButtonRotatingImage )
		m_pButtonRotatingImage->SetPos( iInsetX, iInsetY );
}
...
void CTFButtonBase::SetImageSize( int iWide, int iTall )
{
	...
	if ( m_pButtonRotatingImage )
		m_pButtonRotatingImage->SetSize( iWide, iTall );
}
