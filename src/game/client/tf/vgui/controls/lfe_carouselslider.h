#ifndef __TF_CarouselSlider_H__
#define __TF_CarouselSlider_H__

#include "lfe_hybridbutton.h"

struct TFCarouselSliderInfo_t
{
	CUtlString						m_NameId;
	...
	int								m_nImageId;
	bool							m_bEnabled;
	BaseModHybridButton	*m_pHybridButton;
};

class CTFCarouselSlider : public vgui::Button
{
public:
	DECLARE_CLASS_SIMPLE( CTFCarouselSlider, vgui::Button );

	CTFCarouselSlider( vgui::Panel *pParent, const char *pName );
	virtual ~CTFCarouselSlider();
	...
	int		GetNumGameInfos();
	BaseModHybridButton *GetHybridButton( int nIndex );

private:
	void	SetActiveCarousel( int nActive, bool bKeepFocus );
	...
	CUtlVector< TFCarouselSliderInfo_t >	m_CarouselSliderInfos;
};
