...
CHudMenuSpyDisguise::CHudMenuSpyDisguise( const char *pElementName ) : CHudElement( pElementName ), BaseClass( NULL, "HudMenuSpyDisguise" )
{
	...
	for ( int i=0; i<9; i++ )
	{
		...
		Q_snprintf(buf, sizeof(buf), "class_item_npc_%d", i + 1);
		m_pClassItems_NPC[i] = new EditablePanel(this, buf);
	}
...
void CHudMenuSpyDisguise::ApplySchemeSettings( IScheme *pScheme )
{
	...
	else
	{
		...
		m_pClassItems_NPC[0]->LoadControlSettings("resource/UI/disguise_menu/metropolice.res");
		m_pClassItems_NPC[1]->LoadControlSettings("resource/UI/disguise_menu/combinesoldier.res");
		//m_pClassItems_NPC[2]->LoadControlSettings("resource/UI/disguise_menu/.res");
		m_pClassItems_NPC[3]->LoadControlSettings("resource/UI/disguise_menu/zombiefast.res");
		//m_pClassItems_NPC[4]->LoadControlSettings("resource/UI/disguise_menu/.res");
		//m_pClassItems_NPC[5]->LoadControlSettings("resource/UI/disguise_menu/.res");
		m_pClassItems_NPC[6]->LoadControlSettings("resource/UI/disguise_menu/antlion.res");
		m_pClassItems_NPC[7]->LoadControlSettings("resource/UI/disguise_menu/antlionworker.res");
...
void CHudMenuSpyDisguise::SelectDisguise( int iClass, int iTeam )
{
	...
	if ( pPlayer )
	{
		if ( iTeam == 2 ) // npcs
		{
			if ( iClass == TF_CLASS_SCOUT )	// combine
			{
				iClass = TF_CLASS_COMBINE;
				iTeam = 1;
			}
			else if ( iClass == 2 )
			{
...
void CHudMenuSpyDisguise::FlipAllTeams( void )
{
	// flip the teams
	m_iShowingTeam = (m_iShowingTeam + 1) % 6;

	if ( m_iShowingTeam == 0 || m_iShowingTeam == TF_TEAM_YELLOW )
		m_iShowingTeam = TF_TEAM_RED;
	...
	case TF_TEAM_RED:
		for (int i = 0; i < 9; i++)
		{
			...
			m_pClassItems_NPC[i]->SetVisible(false);
		}
		break;
	...
	case TF_TEAM_GREEN:
		for (int i = 0; i < 9; i++)
		{
			m_pClassItems_Red[i]->SetVisible(false);
			m_pClassItems_Blue[i]->SetVisible(false);
			m_pClassItems_NPC[i]->SetVisible(true);
		}
		break;
	}
}
