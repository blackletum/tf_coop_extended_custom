...
void CTFHudWeaponAmmo::ApplySchemeSettings( IScheme *pScheme )
{
	...
	m_pWeaponLowAmmoImage = dynamic_cast<ImagePanel *>( FindChildByName( "HudWeaponLowAmmoImage" ) );
...
void CTFHudWeaponAmmo::OnThink()
{
	...
	if ( m_flNextThink < gpGlobals->curtime )
	{
		...
		else
		{
			...
			float flAmmo = (float)pPlayer->GetAmmoCount( pWeapon->GetPrimaryAmmoType() ) / (float)pWeapon->GetMaxAmmo();
			if ( flAmmo < 0.25f )
			{
				if ( m_pWeaponLowAmmoImage )
				{
					if ( m_pWeaponLowAmmoImage->IsVisible() )
						m_pWeaponLowAmmoImage->SetVisible( false );
					else
						m_pWeaponLowAmmoImage->SetVisible( true );
				}
			}
			else
			{
				if ( m_pWeaponLowAmmoImage && m_pWeaponLowAmmoImage->IsVisible() )
					m_pWeaponLowAmmoImage->SetVisible( false );
			}
		}
...
CTFHudGrenadeAmmo::CTFHudGrenadeAmmo( const char *pElementName ) : CHudElement( pElementName ), BaseClass( NULL, "HudGrenadeAmmo" )
{
	...
	//hudlcd->SetGlobalStat( "(ammo_grenades1)", "0" );
	//hudlcd->SetGlobalStat( "(ammo_grenades2)", "0" );

	m_pPrimary = NULL;
	m_pPrimaryShadow = NULL;
	m_pSecondary = NULL;
	m_pSecondaryShadow = NULL;
...
void CTFHudGrenadeAmmo::ApplySchemeSettings( IScheme *pScheme )
{
	...
	m_pPrimary = dynamic_cast<CExLabel *>( FindChildByName( "AmmoPrimary" ) );
	m_pPrimaryShadow = dynamic_cast<CExLabel *>( FindChildByName( "AmmoPrimaryShadow" ) );

	m_pSecondary = dynamic_cast<CExLabel *>( FindChildByName( "AmmoSecondary" ) );
	m_pSecondaryShadow = dynamic_cast<CExLabel *>( FindChildByName( "AmmoSecondaryShadow" ) );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFHudGrenadeAmmo::UpdateAmmoLabels( bool bPrimary, bool bSeconary )
{
	if ( m_pPrimary && m_pPrimaryShadow )
	{
		if ( m_pPrimary->IsVisible() != bPrimary )
		{
			m_pPrimary->SetVisible( bPrimary );
			m_pPrimaryShadow->SetVisible( bPrimary );
		}
	}

	if ( m_pSecondary && m_pSecondaryShadow )
	{
		if ( m_pSecondary->IsVisible() != bSeconary )
		{
			m_pSecondary->SetVisible( bSeconary );
			m_pSecondaryShadow->SetVisible( bSeconary );
		}
	}
}
...
void CTFHudGrenadeAmmo::OnThink()
{
	...
	if ( m_flNextThink < gpGlobals->curtime )
	{
		if ( !TFGameRules()->IsGrenadesAllowed() )
		{
			//hudlcd->SetGlobalStat( "(ammo_grenades1)", "n/a" );
		...
		{
			// Get the ammo in our clip.
			int nAmmo1 = pPlayer->GetAmmoCount( LFE_AMMO_GRENADES1 );
			int nAmmo2 = pPlayer->GetAmmoCount( LFE_AMMO_GRENADES2 );

			//hudlcd->SetGlobalStat( "(ammo_grenades1)", VarArgs( "%d", nAmmo1 ) );
			//hudlcd->SetGlobalStat( "(ammo_grenades2)", VarArgs( "%d", nAmmo2 ) );

			if ( m_nAmmo != nAmmo1 || m_nAmmo2 != nAmmo2 )
			{
				m_nAmmo = nAmmo1;
				m_nAmmo2 = nAmmo2;

				UpdateAmmoLabels( true, true );

				SetDialogVariable( "Ammo1", m_nAmmo );
				SetDialogVariable( "Ammo2", m_nAmmo2 );
			}
		}
