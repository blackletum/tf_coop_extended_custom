...
void C_ObjectSentrygun::CreateLaserBeam( void ) 
{
	...
	// create lasers
	/*if ( !m_pLaserSight )
		m_pLaserSight = ParticleProp()->Create( "laser_sight", PATTACH_POINT_FOLLOW, "laser_origin" );*/
	...
	if ( !m_pLaserDot )
	{
		m_pLaserDot = ParticleProp()->Create( "laser_sight_beam_dot", PATTACH_POINT_FOLLOW, "laser_origin" );
		m_pLaserDot->SetControlPoint( 2, vecColor );
	}
}
...
void C_ObjectSentrygun::CreateShield( void ) 
{
	if ( !m_pShield )
	{
		model_t *pModel = (model_t*)engine->LoadModel( SENTRY_SHIELD );
		m_pShield = tempents->SpawnTempModel( pModel, GetAbsOrigin(), GetAbsAngles(), Vector( 0, 0, 0 ), 1, FTENT_NEVERDIE );
	}
	...
	if ( !m_pShieldEffects )
	{
		m_pShieldEffects = ParticleProp()->Create( "turret_shield", PATTACH_ROOTBONE_FOLLOW );
...
void C_ObjectSentrygun::ClientThink( void ) 
{
	if ( m_iState != SENTRY_STATE_WRANGLED )
	{
		...
		if ( m_iState != SENTRY_STATE_WRANGLED_RECOVERY )
			DestroyShield();
	}
	else
	{
		...
		if ( m_pLaserBeam && m_pLaserDot )
		{
			...
			m_pLaserDot->SetControlPoint( 1, m_vecEnd );
		}
	}
...
void C_ObjectSentrygun::DestroyLaserBeam( void ) 
{
	...
	ParticleProp()->StopEmissionAndDestroyImmediately( m_pLaserDot );
	//m_pLaserSight = NULL;
	m_pLaserBeam = NULL;
	m_pLaserDot = NULL;
...
void C_ObjectSentrygun::DestroyShield( void )
{
	...
	if ( m_pShield )
	{
		m_pShield->flags = FTENT_FADEOUT;
		m_pShield->die = gpGlobals->curtime;
		m_pShield->fadeSpeed = 1.0f;
		m_pShield = NULL;
	}
}
