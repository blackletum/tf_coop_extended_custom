...
// Purpose: 
//-----------------------------------------------------------------------------
class C_DispenserTouchTrigger : public C_BaseEntity
{
	DECLARE_CLASS( C_DispenserTouchTrigger, C_BaseEntity );
public:
	DECLARE_CLIENTCLASS();
	//virtual int		InternalDrawModel( int flags );
};

IMPLEMENT_CLIENTCLASS_DT( C_DispenserTouchTrigger, DT_DispenserTouchTrigger, CDispenserTouchTrigger )
END_RECV_TABLE()

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
/*int	C_DispenserTouchTrigger::InternalDrawModel( int flags )
{
	int ret = BaseClass::InternalDrawModel( flags );

	IMaterial *pTexture = NULL;

	pTexture = materials->FindMaterial( "tools/toolstrigger", TEXTURE_GROUP_WORLD, true );
	modelrender->ForcedMaterialOverride( pTexture );
...
IMPLEMENT_CLIENTCLASS_DT(C_ObjectDispenser, DT_ObjectDispenser, CObjectDispenser)
	...
	RecvPropString( RECVINFO( m_szMaterialOverride ) ),
	RecvPropArray2( 
...
// Constructor: 
//-----------------------------------------------------------------------------
CDispenserControlPanel_Custom::CDispenserControlPanel_Custom( vgui::Panel *parent, const char *panelName )
: BaseClass( parent, "CDispenserControlPanel_Custom" ) 
{
...
// Deactivates buttons we can't afford
//-----------------------------------------------------------------------------
void CDispenserControlPanel_Custom::OnTickActive( C_BaseObject *pObj, C_TFPlayer *pLocalPlayer )
{
	BaseClass::OnTickActive( pObj, pLocalPlayer );
	Assert( dynamic_cast<C_ObjectDispenser*>(pObj) );
	C_ObjectDispenser *pDispenser = static_cast<C_ObjectDispenser*>(pObj);

	if ( m_pHTMLPanel && Q_stricmp( pDispenser->GetCustomHTML(), "" ) )
	{
		m_pHTMLPanel->SetVisible( true );
		m_pHTMLPanel->OpenURL( pDispenser->GetCustomHTML(), NULL );
	}
}
