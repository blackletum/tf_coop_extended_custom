...
void CHudBowChargeMeter::FireGameEvent( IGameEvent *event )
{
	...
	{
		C_BaseCombatCharacter *pPlayer = dynamic_cast< C_BaseCombatCharacter * >( cl_entitylist->GetBaseEntity( event->GetInt( "attachedEntity" ) ) );
		C_TFPlayer *pShooter = dynamic_cast<C_TFPlayer *>( cl_entitylist->GetBaseEntity( event->GetInt( "shooter" ) ) );
		if ( pPlayer && pShooter )
		{
			C_TFProjectile_Arrow *pArrow = new C_TFProjectile_Arrow();
			if ( pArrow )
			{
				pArrow->InitializeAsClientEntity( g_pszArrowModels[event->GetInt( "projectileType" )], RENDER_GROUP_OPAQUE_ENTITY );
				...
				pArrow->SetDieTime( gpGlobals->curtime + 15.0f );
