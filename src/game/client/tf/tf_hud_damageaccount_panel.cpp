...
void CDamageAccountPanel::FireGameEvent( IGameEvent *event )
{
	...
	else if ( ( V_strcmp( type, "player_healed" ) == 0 ) || ( V_strcmp( type, "building_healed" ) == 0 ) || ( V_strcmp( type, "npc_healed" ) == 0 ) )
	{
		OnHealed( event );
	}
	else
...
void CDamageAccountPanel::OnHealed( IGameEvent *event )
{
	...
	if ( V_strcmp( event->GetName(), "building_healed" ) == 0 )
	...
	C_BaseEntity *pPatient = ClientEntityList().GetBaseEntity( iPatient );
	if ( !pPatient )
		return;
