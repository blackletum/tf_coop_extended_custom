...
#if defined ( PORTAL ) || defined( TF_CLASSIC_CLIENT )
...
static ConVar lfe_dof( "lfe_dof", "0", FCVAR_ARCHIVE );
#endif
...
CLIENTEFFECT_REGISTER_BEGIN( PrecachePostProcessingEffects )
	...
	CLIENTEFFECT_MATERIAL( "dev/vignette" )
#endif
...
#if defined( PORTAL ) || defined( TF_CLASSIC_CLIENT ) //in portal, we'd like a copy of the front buffer without the gun in it for use with the depth doubler
...
#if defined( PORTAL ) || defined( TF_CLASSIC_CLIENT ) //the depth range hack doesn't work well enough for the portal mod (and messing with the depth hack values makes some models draw incorrectly)
...
#if defined( PORTAL ) || defined( TF_CLASSIC_CLIENT ) //in portal, we can't use the fog volume for the camera since it's almost never in the same fog volume as what's in front of the portal
	if( g_pPortalRender->GetViewRecursionLevel() == 0 )
	{
...
#if defined( PORTAL ) || defined( TF_CLASSIC_CLIENT ) //when rendering portal views point/plane intersections just don't cut it.
	if( g_pPortalRender->GetViewRecursionLevel() != 0 )
...
void CViewRender::ViewDrawScene_PortalStencil( const CViewSetup &viewIn, ViewCustomVisibility_t *pCustomVisibility )
{
	...
	ITexture* pPrimaryWaterReflectionTexture = GetWaterReflectionTexture();
	ITexture* pReplacementWaterReflectionTexture = portalrendertargets->GetWaterReflectionTextureForStencilDepth(iRecursionLevel);

	//get references to refraction textures
	ITexture* pPrimaryWaterRefractionTexture = GetWaterRefractionTexture();
	ITexture* pReplacementWaterRefractionTexture = portalrendertargets->GetWaterRefractionTextureForStencilDepth(iRecursionLevel);

	//swap texture contents for the primary render targets with those we set aside for this recursion level
	if( pReplacementWaterReflectionTexture != NULL && pPrimaryWaterReflectionTexture != NULL )
		pPrimaryWaterReflectionTexture->SwapContents( pReplacementWaterReflectionTexture );

	if( pReplacementWaterRefractionTexture != NULL && pPrimaryWaterRefractionTexture != NULL )
...
#if defined( PORTAL ) || defined( TF_CLASSIC_CLIENT ) //if we're in the portal mod, we need to make a detour so we can render portal views using stencil areas
...
#if defined( TF_CLIENT_DLL ) || defined( TF_CLASSIC_CLIENT )
	bool bVisionOverride = IsLocalPlayerUsingVisionFilterFlags( TF_VISION_FILTER_PYRO ); // Pyro-vision Goggles
