...
CMovieDisplayScreen::~CMovieDisplayScreen( void )
{
	if (m_pVideo != nullptr)
	{
		g_pVideo->DestroyVideoMaterial(m_pVideo);
		m_pVideo = nullptr;
	}
...
void CMovieDisplayScreen::UpdateMovie( void )
{
	...
	{
		m_pVideo->SetPaused(!bScreenActive);
	}
	...
	if ( bScreenActive  )
	{
		if (!m_pVideo->IsVideoPlaying())
			m_pVideo->StartVideo();

		// Update our frame
		if ( m_pVideo->Update() == false )
		{
...
bool CMovieDisplayScreen::BeginPlayback( const char *pFilename )
{
	...
	VideoPlaybackFlags::EVideoPlaybackFlags_t nFlags = VideoPlaybackFlags::NO_AUDIO;	// FIXME: Allow?
	...
	m_pVideo = g_pVideo->CreateVideoMaterial(szMaterialName, pFilename, "GAME", nFlags);
	if (m_pVideo == nullptr )
		return false;

	if ( ( nFlags & VideoPlaybackFlags::NO_AUDIO) != 0 )
	{
	...
	m_pVideo->GetVideoImageSize( &m_playbackInfo.m_nSourceWidth, &m_playbackInfo.m_nSourceHeight );
	m_pVideo->GetVideoTexCoordRange( &m_playbackInfo.m_flU, &m_playbackInfo.m_flV );
	m_playbackInfo.m_pMaterial = m_pVideo->GetMaterial();
...
void CMovieDisplayScreen::Paint( void )
{
	...
	if ( m_bSlaved == false && m_pVideo == nullptr )
	{
		BaseClass::Paint();
		return;
	}
