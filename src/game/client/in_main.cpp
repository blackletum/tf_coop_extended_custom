...
static	kbutton_t	in_use_action;
...
void StartUseActionSlotItem ( const CCommand &args ) {KeyDown(&in_use_action, args[1] );}
void EndUseActionSlotItem ( const CCommand &args ) {KeyUp(&in_use_action, args[1] );}
...
int CInput::GetButtonBits( int bResetState )
{
	...
	CalcButtonBits( bits, IN_USE_ACTION, s_ClearInputState, &in_use_action, bResetState );
...
static ConCommand startaction( "+use_action_slot_item", StartUseActionSlotItem, "Use the item in the action slot." );
static ConCommand endaction( "-use_action_slot_item", EndUseActionSlotItem );
static ConCommand starttaunt( "+taunt", IN_TauntDown );
