...
static ConVar  cl_extrapolate( "cl_extrapolate", "1", FCVAR_CHEAT, "Enable/disable extrapolation if interpolation history runs out." );
	...
	static ConVar  cl_interp_npcs( "cl_interp_npcs", "0.03", FCVAR_USERINFO, "Interpolate NPC positions starting this many seconds in past (or cl_interp, if greater)" );
#else
...
// Purpose: 
//-----------------------------------------------------------------------------
const Vector &C_BaseEntity::GetItemTintColor( void )
{
	static Vector vecColor( GetRenderColor().r, GetRenderColor().g, GetRenderColor().b );
	return vecColor;
}
