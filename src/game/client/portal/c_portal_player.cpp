...
ConVar cl_usepredictedviewmodel("cl_usepredictedviewmodel", "1", FCVAR_ARCHIVE | FCVAR_CLIENTDLL, \
	"If true, use a predicted viewmodel (which disables the 'looking around' motions, which can cause visual issues with some viewmodels.\n");
...
{
	if (GetActiveWeapon() && GetActiveWeapon()->GetWeaponType() == WEAPON_TYPE_PORTALGUN && !cl_usepredictedviewmodel.GetBool() )
	{
