...
void __MsgFunc_EntityPortalled(bf_read &msg)
{
	...
	if( bIsPlayer )
		((C_TFPlayer *)pEntity)->PlayerPortalled( pPortal );
...
const Vector &C_Prop_Portal::GetPortalLocalMins() const
{
...
const Vector &C_Prop_Portal::GetPortalLocalMaxs() const
{
...
void C_Prop_Portal::Simulate()
{
	...
	int iEntsNearPortal = UTIL_EntitiesInSphere( pEntsNearPortal, 1024, GetNetworkOrigin(), m_flHeight, 0, PARTITION_CLIENT_NON_STATIC_EDICTS );
	...
	if( iEntsNearPortal != 0 )
	{
		...
		for( int i = 0; i != iEntsNearPortal; ++i )
		{
			...
			if ( pWeapon )
			{
				C_TFPlayer *pPortalPlayer = ToTFPlayer( pWeapon->GetOwner() );
				if ( pPortalPlayer ) 
				{
	...
	//now, fix up our list of ghosted renderables.
	{
		...
		for( int i = m_hGhostingEntities.Count(); --i >= 0; )
		{
			...
			C_BaseCombatWeapon *pWeapon = dynamic_cast<C_BaseCombatWeapon*>( pEntity );
			if ( pWeapon && ToTFPlayer( pWeapon->GetOwner() ) )
				bIsHeldWeapon = true;
...
void C_Prop_Portal::OnDataChanged( DataUpdateType_t updateType )
{
	...
	if( m_bActivated )
	{
		...
		{			
			...
			Vector vScaledRight = m_vRight * (m_flWidth * 0.95f);
			Vector vScaledUp = m_vUp * (m_flHeight  * 0.95f);
...
bool C_Prop_Portal::ShouldCollide( int collisionGroup, int contentsMask ) const
{
	return false;
}