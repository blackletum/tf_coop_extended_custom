...
//-----------------------------------------------------------------------------
// Purpose: Feeds the server's MapAdd Files to the parameters dialog
//-----------------------------------------------------------------------------
void CVoteSetupDialog::AddVoteIssueParams_MapAddFiles( CUtlStringList &m_VoteSetupMapAddFiles )
{
	m_VoteIssuesMapAddFiles.RemoveAll();
	for ( int index = 0; index < m_VoteSetupMapAddFiles.Count(); index++ )
	{
		m_VoteIssuesMapAddFiles.AddToTail( m_VoteSetupMapAddFiles[index] );
	}
}
...
void CVoteSetupDialog::OnCommand(const char *command)
{
	...
	if ( !V_stricmp( command, "CallVote" ) )
	{
		...
		if ( iSelectedItem >= 0 )
		{
			...
			if ( V_stricmp( "changelevel", szIssueRaw ) == 0 || V_stricmp( "nextlevel", szIssueRaw ) == 0 )
			{
			...
			else if ( V_stricmp( "kick", szIssueRaw ) == 0 )
			{
				// Get selected Player
				int iSelectedParam = m_pVoteParameterList->GetSelectedItem();
				if ( iSelectedParam >= 0 )
			...
			else if ( !V_stricmp( "changedifficulty", szIssueRaw ) )
			{
				...
				if ( nSelectedParam >= 0 )
				{
					...
					if ( iSelectedParam >= 0 )
					{
						...
						{
							// Which Level?
							int iLevel = pParameterKeyValues->GetInt( "index" );
			...
			else if ( !V_stricmp( "changemapadd", szIssueRaw ) )
			{
				int nSelectedParam = m_pVoteParameterList->GetSelectedItem();
				if ( nSelectedParam >= 0 )
				{
					...
					if ( iSelectedParam >= 0 )
					{
						...
						{
							// Which File?
							const char *szFile = pParameterKeyValues->GetString( "Name" );
							Q_snprintf( szVoteCommand, sizeof( szVoteCommand ), "callvote %s %s\n;", szIssueRaw, szFile );
							engine->ClientCmd( szVoteCommand );
						}
					}
		...
		if ( iSelectedItem >= 0 )
		{
			...
			else if ( !V_stricmp( "changedifficulty", pszIssueRaw ) )
			{
				if ( m_pVoteParameterList->GetItemCount() == 0 )
				{
					...
					pOriginal->SetString( "Name", "#LFE_DifficultyOriginal" );
					pOriginal->SetInt( "index", 1 );
					int iOriginal = m_pVoteParameterList->AddItem( 0, pOriginal );

					KeyValues *pMedium = new KeyValues( "medium" );
					pMedium->SetString( "Name", "#LFE_DifficultyMedium" );
					pMedium->SetInt( "index", 2 );
					int iMedium = m_pVoteParameterList->AddItem( 0, pMedium );

					KeyValues *pHard = new KeyValues( "hard" );
					pHard->SetString( "Name", "#LFE_DifficultyHard" );
					pHard->SetInt( "index", 3 );
					int iHard = m_pVoteParameterList->AddItem( 0, pHard );

					KeyValues *pUnusual = new KeyValues( "unusual" );
					pUnusual->SetString( "Name", "#LFE_DifficultyUnusual" );
					pUnusual->SetInt( "index", 4 );
					int iUnusual = m_pVoteParameterList->AddItem( 0, pUnusual );
					...
					if ( m_hIssueFont != INVALID_FONT )
					{
						m_pVoteParameterList->SetItemFont( iOriginal, m_hIssueFont );
						m_pVoteParameterList->SetItemFgColor( iOriginal, m_IssueFGColor );
						m_pVoteParameterList->SetItemFont( iMedium, m_hIssueFont );
						m_pVoteParameterList->SetItemFgColor( iMedium, m_IssueFGColor );
						m_pVoteParameterList->SetItemFont( iHard, m_hIssueFont );
						m_pVoteParameterList->SetItemFgColor( iHard, m_IssueFGColor );
						m_pVoteParameterList->SetItemFont( iUnusual, m_hIssueFont );
						m_pVoteParameterList->SetItemFgColor( iUnusual, m_IssueFGColor );
					}
				}
			}
			else if ( !V_stricmp( "changemapadd", pszIssueRaw ) )
			{
				// Feed the mapadd files to the parameters list
				for ( int index = 0; index < m_VoteIssuesMapAddFiles.Count(); index++ )
				{
					// Don't show the current mapadd file
					const char *pszMapAddFileName = lfe_mapadd_file.GetString();
					if ( !pszMapAddFileName || !pszMapAddFileName[0] )
					{
						// Use the map name
						char szShortMapName[ MAX_MAP_NAME ];
						V_strncpy( szShortMapName, engine->GetLevelName(), sizeof( szShortMapName ) );
						...
						if ( V_strncmp( m_VoteIssuesMapAddFiles[index], V_GetFileName( szShortMapName ), ( V_strlen( m_VoteIssuesMapAddFiles[index] ) - 1 ) ) == 0 )
							continue;
					}
					else
					{
						// Use the specified mapadd file
						if ( V_strncmp( m_VoteIssuesMapAddFiles[index], lfe_mapadd_file.GetString(), ( V_strlen( m_VoteIssuesMapAddFiles[index] ) - 1 ) ) == 0 )
							continue;
					}

					KeyValues *pKeyValues = new KeyValues( "Name" );
					pKeyValues->SetString( "Name", m_VoteIssuesMapAddFiles[index] );
					pKeyValues->SetInt( "index", index );
					int iId = m_pVoteParameterList->AddItem( 0, pKeyValues );
					pKeyValues->deleteThis();
				...
				{
					KeyValues *pKeyValues = new KeyValues( "Name" );
					pKeyValues->SetString( "Name", "#Lfe_vote_no_mapadd" );
					pKeyValues->SetInt( "index", 1 );
					m_pVoteParameterList->AddItem( 0, pKeyValues );
					pKeyValues->deleteThis();
				}
...
void CHudVote::MsgFunc_VoteSetup( bf_read &msg )
{
	...
	m_VoteSetupMapAddFiles.RemoveAll();
	if ( g_pStringTableServerMapAddFiles )
	{
		int index = g_pStringTableServerMapAddFiles->FindStringIndex( "ServerMapAddFiles" );
		if ( index != ::INVALID_STRING_INDEX )
		{
			int nLength = 0;
			const char *pszMapAddFiles = (const char *)g_pStringTableServerMapAddFiles->GetStringUserData( index, &nLength );
			if ( pszMapAddFiles && pszMapAddFiles[0] )
			{
				if ( pszMapAddFiles && nLength )
				{
					V_SplitString( pszMapAddFiles, "\n", m_VoteSetupMapAddFiles );
				}

				// Alphabetize
				if ( m_VoteSetupMapAddFiles.Count() )
				{
					m_VoteSetupMapAddFiles.Sort( m_VoteSetupMapAddFiles.SortFunc );
				}
			}
		}
	}
...
void CHudVote::PropagateOptionParameters( void )
{
	...
	m_pVoteSetupDialog->AddVoteIssueParams_MapAddFiles( m_VoteSetupMapAddFiles );
