...
void OnCaptionLanguageChanged( IConVar *pConVar, const char *pOldString, float flOldValue )
{
	...
	Q_snprintf(fn2, sizeof(fn2), "resource/tf_coop_extended_beta_%s.txt", var.GetString());
	...
	{
		g_pVGuiLocalize->AddFile( "resource/closecaption_%language%.txt", "GAME", true );
		g_pVGuiLocalize->AddFile("resource/tf_coop_extended_beta_%language%.txt", "GAME", true);
	...
	if ( Q_strlen( var.GetString() ) > 0 && Q_stricmp( var.GetString(), uilanguage ) )
	{
		if ( !IsX360() )
		{
			...
			{
				char dbfile[512];
				Q_snprintf(dbfile, sizeof(dbfile), "resource/tf_coop_extended_beta_%s.txt", var.GetString());
			...
			{
				char fallback[512];
				Q_snprintf(fallback, sizeof(fallback), "resource/tf_coop_extended_beta_%s.txt", uilanguage);
