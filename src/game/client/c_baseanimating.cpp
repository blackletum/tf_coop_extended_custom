...
void C_ClientRagdoll::FireGameEvent( IGameEvent *event )
{
...
{
	if ( Q_stricmp( m_szMaterialOverride, "" ) )
	{
		m_MaterialOverride.Init( m_szMaterialOverride, "ClientEffect textures", true );
...
void C_BaseAnimating::OnDataChanged( DataUpdateType_t updateType )
{
	...
	{
		if ( ( m_pGlowEffect->GetColor().x != m_vGlowColor.x ) ||
			( m_pGlowEffect->GetColor().y != m_vGlowColor.y ) ||
			( m_pGlowEffect->GetColor().z != m_vGlowColor.z ) )
			UpdateGlowEffect();
	}
#endif // GLOWS_ENABLE
}
...
// Purpose:
//------------------------------------------------------------------------------
void C_BaseAnimating::MaterialOverride( const char *pMaterial )
{
	Q_snprintf( m_szMaterialOverride, MAX_PATH, pMaterial );
}