...
void DoEnginePostProcessing( int x, int y, int w, int h, bool bFlashlightIsOn, bool bPostVGui )
{
	...
	if ( mat_vignette_enable.GetBool() || IsLocalPlayerUsingVisionFilterFlags( TF_VISION_FILTER_HALLOWEEN ) )
	{
		static IMaterial *vignetteMat = materials->FindMaterial( "dev/vignette", TEXTURE_GROUP_OTHER );
		if ( vignetteMat )
		{
