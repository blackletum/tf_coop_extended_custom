...
INetworkStringTable* g_pStringTableServerMapAddFiles = NULL;
...
void CHLClient::LevelInitPreEntity( char const* pMapName )
{
	...
	AddRequiredMapSearchPaths( pMapName );
...
void CHLClient::ResetStringTablePointers()
{
	...
	g_pStringTableServerMapAddFiles = NULL;
...
void CHLClient::InstallStringTableCallback( const char *tableName )
{
	...
	else if ( !Q_strcasecmp( tableName, "ServerMapAddFiles" ) )
	{
		g_pStringTableServerMapAddFiles = networkstringtable->FindTable( tableName );
	}
#ifdef TF_CLIENT_DLL
...
void CHLClient::ClientAdjustStartSoundParams( StartSoundParams_t& params )
{
	...
	// A player speaking
	if ( params.entchannel == CHAN_VOICE && GameRules() && pEntity && ( pEntity->IsPlayer() || pEntity->IsNPC() ) )
	{
