...
void CBaseEntity::FireBullets( const FireBulletsInfo_t &info )
{
	...
	if ( pAmmoDef->Index( "SniperRound" ) )
		g_MultiDamage.SetDamageCustom( TF_DMG_CUSTOM_HEADSHOT );
...
#else
			UTIL_TraceHull( info.m_vecSrc, vecEnd, Vector( -3, -3, -3 ), Vector( 3, 3, 3 ), MASK_SHOT, &traceFilter, &tr );
#endif //#ifdef PORTAL
		}
		else
		{
			...
			if ( TFGameRules() && TFGameRules()->GameModeUsesUpgrades() )
			{
				...
				UTIL_TraceLine(info.m_vecSrc, vecEnd, MASK_SHOT, &traceFilterChain, &tr);
			}
			else
			{
	...
	const Vector &vecEnd, ITraceFilter *pTraceFilter, Vector *pVecTracerDest )
{
	...
	UTIL_TraceLine( info.m_vecSrc, vecEnd, (MASK_SHOT|CONTENTS_WATER|CONTENTS_SLIME), pTraceFilter, &waterTrace );
...
{
#if !defined( HL2MP ) && !defined( TF_CLASSIC )
