...
{
	if (V_strcmp(GetWpnData().szWorldModelHL1, ""))
	{
		ConVarRef sv_hl1_hd("sv_hl1_hd");
		if (sv_hl1_hd.GetBool() && V_strcmp(GetWpnData().szWorldModelHL1HD, ""))
		{
			return GetWpnData().szWorldModelHL1HD;
		}
		else
		{
			return GetWpnData().szWorldModelHL1;
		}
	}
...
{
	float flMaxClip = (float)GetWpnData().iMaxClip1;
...
{
	float flDefaultClip = (float)GetWpnData().iDefaultClip1;
...
void CBaseCombatWeapon::OnPickedUp( CBaseCombatCharacter *pNewOwner )
{
	...
	CTFPlayer *pTFOwner = ToTFPlayer( pNewOwner );
	if ( pTFOwner && pTFOwner->IsHoldingEntity( this ) )
		pTFOwner->ForceDropOfCarriedPhysObjects( this );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseCombatWeapon::ReapplyProvision( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseCombatWeapon::OnActiveStateChanged( int iOldState )
{
	CBaseCombatCharacter *pOwner = GetOwner();
	if ( pOwner )
	{
...
{ 
	//Adrian: Oh man...
#if !defined( CLIENT_DLL ) && (defined( HL2MP ) || defined( PORTAL )) && !defined( TF_CLASSIC )
	SetModel( GetWorldModel() );
#endif
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CBaseCombatWeapon::Deploy( void )
{
...
{
	int iUseAutoFireFullClip = 0;
	CALL_ATTRIB_HOOK_INT( iUseAutoFireFullClip, auto_fires_full_clip );
	return ( iUseAutoFireFullClip > 0 );
}

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
bool CBaseCombatWeapon::AutoFiresFullClipAllAtOnce( void ) const
{
	return false;
}
...
//-----------------------------------------------------------------------------
// Purpose: Primary fire all from the clip
//-----------------------------------------------------------------------------
void CBaseCombatWeapon::FireFullClipAtOnce( void )
{
	// If my clip is empty (and I use clips) start reload
	if ( UsesClipsForAmmo1() && !m_iClip1 ) 
	{
...
// Purpose:
// -----------------------------------------------------------------------------
bool CBaseCombatWeapon::IsWeapon( int iWeapon ) const
{
...
{
	if ( GetWeaponID() == TF_WEAPON_PISTOL ||
		 GetWeaponID() ==TF_WEAPON_PISTOL_SCOUT ||
		 GetWeaponID() == TF_WEAPON_HL2_PISTOL ||
		 GetWeaponID() == TF_WEAPON_HL2_357 ||
		 GetWeaponID() == TF_WEAPON_HL2_ALYXGUN )
	{
		return true;	 
	}
...
{
	if ( GetWeaponID() == TF_WEAPON_SHOTGUN_PRIMARY ||
		 GetWeaponID() == TF_WEAPON_SHOTGUN_SOLDIER ||
		 GetWeaponID() == TF_WEAPON_SHOTGUN_HWG ||
		 GetWeaponID() == TF_WEAPON_SHOTGUN_PYRO ||
		 GetWeaponID() == TF_WEAPON_SCATTERGUN ||
		 GetWeaponID() == TF_WEAPON_TFC_SHOTGUN ||
		 GetWeaponID() == TF_WEAPON_TFC_SUPER_SHOTGUN ||
		 GetWeaponID() == TF_WEAPON_HL2_SHOTGUN ||
		 GetWeaponID() == TF_WEAPON_HL2_ANNABELLE )
	{
		return true;	 
	}
...
// Purpose:
// -----------------------------------------------------------------------------
bool CBaseCombatWeapon::IsSMG( void )
{
	if ( GetWeaponID() == TF_WEAPON_SMG ||
		 GetWeaponID() == TF_WEAPON_TFC_AUTOMATICRIFLE ||
		 GetWeaponID() == TF_WEAPON_HL2_SMG1 ||
		 GetWeaponID() == TF_WEAPON_HL2_SMG2 )
	{
		return true;	 
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CBaseCombatWeapon::IsEnergyWeapon( void ) const
{
	int iEnergyNoAmmo = 0;
	CALL_ATTRIB_HOOK_INT( iEnergyNoAmmo, energy_weapon_no_ammo );
	return ( iEnergyNoAmmo > 0 );
}

//-----------------------------------------------------------------------------
// Purpose:
// ----------------------------------------------------------------------------
bool CBaseCombatWeapon::IsHonorBound( void ) const
{
	int iHonorBound = 0;
...
// Purpose:
// ----------------------------------------------------------------------------
bool CBaseCombatWeapon::IsPassiveWeapon( void ) const
{
	return false;
}
...
// ----------------------------------------------------------------------------
bool CBaseCombatWeapon::IsSilentKiller( void ) const
{
	int nSilentKiller = 0;
