...
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseGrenade::RemoveGrenade( bool bBlinkOut )
{
	// Kill it
	...
	{
		// Sprite flash
		CSprite *pGlowSprite = CSprite::SpriteCreate( "sprites/light_glow02_noz.vmt", GetAbsOrigin(), false );
		if ( pGlowSprite )
		{
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseGrenade::Destroy( bool bBlinkOut, bool bBreakRocket )
{
	if ( bBlinkOut )
	{
		RemoveGrenade();
...
void CBaseGrenade::Detonate( void )
{
	...
	if( tr.startsolid )
	{
		...
		Ray_t ray2; ray2.Init( GetAbsOrigin(), GetAbsOrigin() + Vector( 0, 0, -32) );
		UTIL_Portal_TraceRay( ray2, MASK_SHOT_HULL, this, COLLISION_GROUP_NONE, &tr );
	}
...
void CBaseGrenade::ExplodeTouch( CBaseEntity *pOther )
{
	...
	UTIL_Portal_TraceRay( ray, MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr );
