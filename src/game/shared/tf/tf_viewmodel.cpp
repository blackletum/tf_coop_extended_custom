...
int CTFViewModel::GetSkin()
{
	...
	if ( pPlayer )
	{
		nSkin = pWeapon->GetSkin();
		if ( pWeapon->GetTFWpnData().m_bHasTeamSkins_Viewmodel && ( pPlayer->GetTeamNumber() == TF_TEAM_BLUE ) )
			nSkin += 1;
	}
...
//-----------------------------------------------------------------------------
// Vintage Purpose: Update minimal viewmodel positions
//-----------------------------------------------------------------------------
void CTFViewModel::CalcMinViewmodelOffset( C_TFPlayer *owner )
{
	...
	if ( pWeapon )
	{
		...
		{
			ConDMsg( "No offset specified for minimal viewmodel. Defaulting to 0\n" );
		}
