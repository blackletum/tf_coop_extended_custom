...
{
	if ( TFGameRules() && TFGameRules()->IsTFCAllowed() )
	{
		if ( m_szTFCModelName[0] != '\0') 
			return m_szTFCModelName;
	}

	if ( TFGameRules() && TFGameRules()->IsMvMModelsAllowed() )
	{
		if ( m_szBotModelName[0] != '\0') 
			return m_szBotModelName;
	}
...
void TFPlayerClassData_t::ParseData( KeyValues *pKeyValuesData )
{
	...
	Q_strncpy( m_szTFCModelName, pKeyValuesData->GetString( "model_tfc" ), TF_NAME_LENGTH );
	Q_strncpy( m_szBotModelName, pKeyValuesData->GetString( "model_bot" ), TF_NAME_LENGTH );
	...
	int i;
	char buf[32];
	for ( i=0;i<LOADOUT_POSITION_BUFFER;i++ )
	{
