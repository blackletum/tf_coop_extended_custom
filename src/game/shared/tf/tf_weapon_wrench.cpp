...
void CTFWrench::OnFriendlyNPCHit( CAI_BaseNPC *pNPC, CTFPlayer *pPlayer, Vector vecHitPos )
{
	// Did this npc hit do any work? repair or upgrade?
	bool bUsefulHit = pNPC->InputWrenchHit( pPlayer, this, vecHitPos );
...
void CTFWrench::Smack( void )
{
	...
	if ( ( trace.fraction < 1.0f && trace.m_pEnt ) && trace.m_pEnt->InSameTeam( pPlayer ) )
	{
		...
		else if ( trace.m_pEnt->IsNPC() && trace.m_pEnt->MyNPCPointer()->CanUpgrade() )
		{
#ifdef GAME_DLL
			OnFriendlyNPCHit( dynamic_cast< CAI_BaseNPC * >( trace.m_pEnt ), pPlayer, trace.endpos );
#endif
		}
		else
		{
...
// Purpose: 
//-----------------------------------------------------------------------------
float CTFWrench::GetMeleeDamage( CBaseEntity *pTarget, int &iDamageType, int &iCustomDamage )
{
...
CREATE_SIMPLE_WEAPON_TABLE( TFCSpanner, tf_weapon_tfc_spanner )