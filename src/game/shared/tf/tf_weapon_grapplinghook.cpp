...
void CTFGrapplingHook::SecondaryAttack( void )
{
	...
	if ( pOwner->m_Shared.CanRuneCharge() )
		ActivateRune();
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFGrapplingHook::ActivateRune( void )
{
	CTFPlayer *pOwner = GetTFPlayerOwner();
	if ( !pOwner )
		return;

	if ( pOwner->m_Shared.GetRuneProgress() < 100.0f )
		return;

	pOwner->m_Shared.RemoveCond( TF_COND_RUNE_SUPERNOVA );
}
...
bool CTFGrapplingHook::CanAttack( void )
{
	...
	UTIL_Portal_TraceRay( ray, MASK_SOLID|CONTENTS_HITBOX, this, COLLISION_GROUP_NONE, &tr );
	if ( tr.fraction > 1.0f )
		return false;
