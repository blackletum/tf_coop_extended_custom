...
// Do we have to be built on an attachment point
//-----------------------------------------------------------------------------
bool CBaseObject::CanBeBuiltOnAttachmentPoint( void ) const
{
	return (m_fObjectFlags & OF_CAN_BE_BUILT_ON_ATTACHMENT) != 0;
}
...
bool CBaseObject::CalculatePlacementPos( void )
{
	...
	for ( iIteration = 0; iIteration < nIterations; iIteration++ )
	{
		UTIL_TraceHull( 
			...
			-vHalfBuildDimsXY, vHalfBuildDimsXY, MASK_SOLID /*MASK_PLAYERSOLID_BRUSHONLY*/, this, COLLISION_GROUP_PLAYER_MOVEMENT, &tr );
		bottomZ = tr.endpos.z;
...
bool CBaseObject::VerifyCorner( const Vector &vBottomCenter, float xOffset, float yOffset )
{
	...
	trace_t tr;
	Ray_t ray; ray.Init( vStart, vStart - Vector( 0, 0, TF_OBJ_GROUND_CLEARANCE ) );
	UTIL_Portal_TraceRay( ray, MASK_PLAYERSOLID_BRUSHONLY, this, COLLISION_GROUP_PLAYER_MOVEMENT, &tr );
...
bool CBaseObject::IsPlacementPosValid( void )
{
	...
	Ray_t ray; ray.Init( pPlayer->EyePosition(), m_vecBuildOrigin + Vector(0,0,m_vecBuildMaxs[2] * 0.5) );
	UTIL_Portal_TraceRay( ray, MASK_PLAYERSOLID_BRUSHONLY, pPlayer, COLLISION_GROUP_NONE, &tr );
	if ( tr.fraction < 1.0 )
	{
		return false;
	}
