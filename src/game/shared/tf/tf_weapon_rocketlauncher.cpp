...
// Purpose:
// ----------------------------------------------------------------------------
void CTFRocketLauncher_AirStrike::OnPlayerKill( CBaseEntity *pVictim, const CTakeDamageInfo &info )
{
	CTFPlayer *pOwner = GetTFPlayerOwner();
	if ( !pOwner )
		return;
	...
	int iKillCount = Min( GetCount() + 1, 4 );

	// wiki said that airstrike is just eyelander but for soldier.
	CTFPlayer *pPlayer = ToTFPlayer( pVictim );
	if ( pPlayer )
		iKillCount += pPlayer->m_Shared.GetDecapitationCount();

	pOwner->m_Shared.SetDecapitationCount( iKillCount );
}