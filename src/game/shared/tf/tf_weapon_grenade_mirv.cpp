...
#define GRENADE_MODEL "models/tfc/mirv_grenade.mdl"
#define GRENADE_SOUND "Weapon_Grenade_TFC.Timer"

#define GRENADE_MODEL_BOMBLET "models/tfc/bomblet.mdl"
...
LINK_ENTITY_TO_CLASS( tf_weapon_grenade_mirv, CTFGrenadeMirvProjectile );
...
{
	CTFGrenadeMirvProjectile *pGrenade = static_cast<CTFGrenadeMirvProjectile *>( CTFWeaponBaseGrenadeProj::Create( "tf_weapon_grenade_mirv", position, angles, velocity, angVelocity, pOwner, pWeapon ) );
	if ( pGrenade )
							                    ...
							                    const AngularImpulse &angVelocity, CBaseCombatCharacter *pOwner, float timer )
{
	...
	if ( pBomb )
	{
		...
		{
			pBomb->SetAbsVelocity( velocity );
			pBomb->ApplyLocalAngularVelocityImpulse( angVelocity );
		}
	}
