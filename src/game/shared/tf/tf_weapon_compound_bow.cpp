...
void CTFCompoundBow::Precache( void )
{
	...
	PrecacheParticleSystem( "pyrovision_v_flaming_arrow" );
	PrecacheParticleSystem( "flaming_arrow" );
	PrecacheParticleSystem( "pyrovision_flaming_arrow" );
...
//-----------------------------------------------------------------------------
void CTFCompoundBow::SetArrowAlight( bool bLit )
{
	if ( bLit )
	{
...
{
	float flTime = TF_BOW_MAX_CHARGE_TIME;
	CTFPlayer *pPlayer = GetTFPlayerOwner();
