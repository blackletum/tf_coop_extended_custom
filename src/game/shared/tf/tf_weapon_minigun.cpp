...
#define TF_ATTACK_PROJECTILE_RADIUS	300.0f
...
//-----------------------------------------------------------------------------
// Purpose: sigsegv said there's sphere in front of heavy
//-----------------------------------------------------------------------------
void CTFMinigun::AttackEnemyProjectiles( void )
{
	...
	int iAttackProjectile = 0;
	CALL_ATTRIB_HOOK_INT( iAttackProjectile, attack_projectiles );
	if ( iAttackProjectile == 0 )
		return;

 	CBaseEntity *pEntity = NULL;
	Vector vecOrigin = pOwner->Weapon_ShootPosition();

	if ( tf_debug_damage.GetBool() )
		NDebugOverlay::Sphere( vecOrigin, vec3_angle, TF_ATTACK_PROJECTILE_RADIUS, 0, 255, 0, 0, false, 0 );

 	for ( CEntitySphereQuery sphere( vecOrigin, TF_ATTACK_PROJECTILE_RADIUS ); ( pEntity = sphere.GetCurrentEntity() ) != NULL; sphere.NextEntity() )
	{
		if ( !pEntity )
			continue;
		...
		Vector vecDir = vecHitPoint - vecOrigin;
 		if ( vecDir.LengthSqr() < ( TF_ATTACK_PROJECTILE_RADIUS * TF_ATTACK_PROJECTILE_RADIUS ) )
		{
			Vector vecToPlayer = pEntity->WorldSpaceCenter() - pOwner->EyePosition();
			...
			float flDistance = ( pOwner->GetAbsOrigin() - pEntity->GetAbsOrigin() ).Length();

			if ( vecToPlayer.Dot( vecFwd ) > 0.975f || flDistance < 100.0f )
			{
				// hurt
				CBaseProjectile *pProj = dynamic_cast<CBaseProjectile *>( pEntity );
				if ( pProj && pProj->IsDestroyable() )
				{
					if ( iAttackProjectile == 1 )
					{
						pProj->IncrementDestroyableHitCount();
					}
					else if ( iAttackProjectile >= 2 )
					{
						pProj->IncrementDestroyableHitCount();
						pProj->IncrementDestroyableHitCount();
					}
				...
				// die
				if ( pEntity->ClassMatches( "rpg_missile" ) || pEntity->ClassMatches( "apc_missile" ) || pEntity->ClassMatches( "rpg_rocket" ) || pEntity->ClassMatches( "hunter_flechette" ) )
				{
					pEntity->EmitSound( "Halloween.HeadlessBossAxeHitWorld" );
					CPVSFilter filter( pEntity->GetAbsOrigin() );
					UserMessageBegin( filter, "BreakModelRocketDud" );
						WRITE_SHORT( pEntity->GetModelIndex() );
						WRITE_VEC3COORD( pEntity->GetAbsOrigin() );
						WRITE_ANGLES( pEntity->GetAbsAngles() );
					MessageEnd();
					UTIL_Remove( pEntity );
				}

				// begone
				if ( pEntity->ClassMatches( "crossbow_bolt" ) || pEntity->ClassMatches( "crossbow_bolt_hl1" ) )
				{
					UTIL_Remove( pEntity );
				}
			}
	...
	case AC_STATE_STARTFIRING:
		{
			...
			if ( m_flNextPrimaryAttack <= gpGlobals->curtime )
			{
				if ( m_iWeaponMode == TF_WEAPON_SECONDARY_MODE )
				{
					...
					{
						CSoundEnt::InsertSound( SOUND_COMBAT, GetAbsOrigin(), 600, 1.0, pPlayer, SOUNDENT_CHANNEL_REPEATING );
					}
#endif
				}
				else
	...
	case AC_STATE_SPINNING:
		{
			...
			if ( m_iWeaponMode == TF_WEAPON_PRIMARY_MODE )
			{
				if ( pPlayer->GetAmmoCount(m_iPrimaryAmmoType) > 0 )
				{
					...
					if ( nMinigunNoSound != 1 )
					{
						CSoundEnt::InsertSound( SOUND_COMBAT, GetAbsOrigin(), SOUNDENT_VOLUME_MACHINEGUN, 1.0, pPlayer, SOUNDENT_CHANNEL_REPEATING );
					}
#endif
					m_iWeaponState = AC_STATE_FIRING;
				}
