...
void CTeamplayRoundBasedRules::CheckRestartRound( void )
{
	...
	if ( iRestartDelay > 0 || bRestartGameNow )
	{
		...
		if ( TFGameRules() && ( TFGameRules()->IsHordeMode() ) )
		{
			iDelayMax = 120;
		}
...
void CTeamplayRoundBasedRules::State_Think_RND_RUNNING( void )
{
	...
	if ( TFGameRules()->IsCoOpGameRunning() || TFGameRules()->IsVersus() || TFGameRules()->IsHordeMode() )
	{
...
void CTeamplayRoundBasedRules::BalanceTeams( bool bRequireSwitcheesToBeDead )
{
	...
	{
		iNumSwitchesRequired = lfe_vs_min_red_players.GetInt() - pLightTeam->GetNumPlayers();
	}
	else
	{
...
bool CTeamplayRoundBasedRules::WouldChangeUnbalanceTeams( int iNewTeam, int iCurrentTeam  )
{
	...
	if ( TFGameRules()->IsCoOp() || TFGameRules()->IsHordeMode() )
	{
...
/*
	...
	if ( TFGameRules()->IsVersus() )
	{
		...
		if ( iRebelPlayers < lfe_vs_min_red_players.GetInt() )
			return true;
...
bool CTeamplayRoundBasedRules::AreTeamsUnbalanced( int &iHeaviestTeam, int &iLightestTeam )
{
	...
	if ( TFGameRules()->IsVersus() )
	{
		...
		if ( pRebels->GetNumPlayers() < lfe_vs_min_red_players.GetInt() && pCombine->GetNumPlayers() > 0 )
		{
