...
//=============================================================================
	...
	*/
	...
	void CMultiplayRules::LoadMapCycleFile( void )
	{
		...
		{
			// Search for all mapadd files that are prefixed with the current map name
			CUtlString sFileList;

			char szBaseName[_MAX_PATH];
			V_snprintf( szBaseName, sizeof( szBaseName ), "maps/mapadd/%s*.txt", STRING(gpGlobals->mapname) );

			FileFindHandle_t addHandle;
			const char *pMapAddFileName = filesystem->FindFirst( szBaseName, &addHandle );

			while ( pMapAddFileName && pMapAddFileName[ 0 ] != '\0' )
			{
				// Skip it if it's a directory or is the folder info
				if ( filesystem->FindIsDirectory( addHandle ) )
				{
					pMapAddFileName = filesystem->FindNext( addHandle );
					continue;
				}

				const char *pchMapAddPostfix = StringAfterPrefix( pMapAddFileName, STRING(gpGlobals->mapname) );
				if ( pchMapAddPostfix )
				{
					char szShortName[_MAX_PATH];
					V_strncpy( szShortName, ( ( pchMapAddPostfix[ 0 ] == '_' ) ? ( pchMapAddPostfix + 1 ) : "default" ), sizeof( szShortName ) ); // skip the '_'
					V_StripExtension( szShortName, szShortName, sizeof( szShortName ) );
			...
			filesystem->FindClose( addHandle );

			if ( sFileList.Length() > 0 )
			{
				g_pStringTableServerMapAddFiles->AddString( CBaseEntity::IsServer(), "ServerMapAddFiles", sFileList.Length() + 1, sFileList.String() );
			}
		}
#endif
	...
	void CMultiplayRules::LoadVoiceCommandScript( void )
	{
		...
		if ( !pKV->LoadFromFile( filesystem, "scripts/voicecommands.txt", "MOD" ) )
			return;
