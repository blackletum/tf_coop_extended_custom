	...
	{	
		...
		if( m_pLinkedPortal && m_pLinkedPortal->m_InternalData.Simulation.pPhysicsEnvironment )
		{
			...
			if( pClone )
			{
				...
				CTFPlayer *pPlayer = (CTFPlayer *)GetPlayerHoldingEntity( pEntity );

				if ( !pPlayer && pEntity->IsPlayer() )
				{
					pPlayer = (CTFPlayer *)pEntity;
				}
...
void CPortalSimulator::CreateLocalPhysics( void )
{
	...
	//World
	{
		...
		Assert(m_InternalData.Simulation.Static.World.Displacements.pPhysicsObject == NULL); //Be sure to find graceful fixes for asserts, performance is a big concern with portal simulation
...
void CPortalSimulator::CreateLinkedPhysics( void )
{
	...
	Assert(m_InternalData.Simulation.Static.Wall.RemoteTransformedToLocal.Displacements.pPhysicsObject == NULL); //Be sure to find graceful fixes for asserts, performance is a big concern with portal simulation
...
void CPortalSimulator::CreateLocalCollision( void )
{
	...
	CREATEDEBUGTIMER(worldTerrainTimer);
	STARTDEBUGTIMER(worldTerrainTimer);
	Assert(m_InternalData.Simulation.Static.World.Displacements.pCollideable == NULL); //Be sure to find graceful fixes for asserts, performance is a big concern with portal simulation
	...
	STOPDEBUGTIMER(worldTerrainTimer);
	DEBUGTIMERONLY(DevMsg(2, "[PSDT:%d] %sWorld Displacements=%fms\n", GetPortalSimulatorGUID(), TABSPACING, worldTerrainTimer.GetDuration().GetMillisecondsF()); );
...
void CPortalSimulator::CreatePolyhedrons( void )
{
	...
	//World
	{
		...
		// Displacements
		{
			Assert(m_InternalData.Simulation.Static.World.Displacements.Polyhedrons.Count() == 0);

			CPhysCollide* pDispCollide = enginetrace->GetCollidableFromDisplacementsInAABB(vAABBMins, vAABBMaxs);
			if (pDispCollide)
			{
				CPhysConvex* ConvexesArray[1024];
				int iConvexes = physcollision->GetConvexesUsedInCollideable(pDispCollide, ConvexesArray, 1024);
				Assert(iConvexes < 1024);
				...
				for (int j = 0; j != iConvexes; ++j)
				{
					...
					if (pTempPolyhedron)
					{
						CPolyhedron* pClippedPropPolyhedron = ClipPolyhedron(pTempPolyhedron, fWorldClipPlane_Reverse, 1, PORTAL_POLYHEDRON_CUT_EPSILON, false);
...
void CPortalSimulator::PostPhysFrame( void )
{
	if (AI_IsSinglePlayer())
	{
		...
		{
			CTFPlayer* pPlayer = dynamic_cast<CTFPlayer*>(UTIL_GetLocalPlayer());
...
bool CPortalSimulator::CreatedPhysicsObject( const IPhysicsObject *pObject, PS_PhysicsObjectSourceType_t *pOut_SourceType ) const
{
	...
	{
		if (pOut_SourceType)
			*pOut_SourceType = PSPOST_LOCAL_TERRAIN;

		return true;
	}

	if (pObject == m_InternalData.Simulation.Static.Wall.RemoteTransformedToLocal.Displacements.pPhysicsObject)
	{
		if (pOut_SourceType)
			*pOut_SourceType = PSPOST_REMOTE_TERRAIN;
