...
void CGameRules::RadiusDamage( const CTakeDamageInfo &info, const Vector &vecSrcIn, float flRadius, int iClassIgnore, CBaseEntity *pEntityIgnore )
{
	...
	for ( CEntitySphereQuery sphere( vecSrc, flRadius ); (pEntity = sphere.GetCurrentEntity()) != NULL; sphere.NextEntity() )
	{
		...
		Ray_t ray; ray.Init( vecSrc, vecSpot );
		UTIL_Portal_TraceRay( ray, MASK_RADIUS_DAMAGE, info.GetInflictor(), COLLISION_GROUP_NONE, &tr );
			...
			if ( tr.fraction != 1.0 )
			{
				if ( IsExplosionTraceBlocked(&tr) )
				{
					if( ShouldUseRobustRadiusDamage( pEntity ) )
					{
						...
						Ray_t ray2; ray2.Init( tr.endpos, tr.endpos + vecDeflect * ROBUST_RADIUS_PROBE_DIST );
						UTIL_Portal_TraceRay( ray2, MASK_RADIUS_DAMAGE, info.GetInflictor(), COLLISION_GROUP_NONE, &tr );
						...
						Ray_t ray3; ray3.Init( tr.endpos, vecSpot );
						UTIL_Portal_TraceRay( ray3, MASK_RADIUS_DAMAGE, info.GetInflictor(), COLLISION_GROUP_NONE, &tr );
				...
				if( tr.m_pEnt && tr.m_pEnt != pEntity && tr.m_pEnt->GetOwnerEntity() != pEntity )
				{
					...
					Ray_t ray4; ray4.Init( vecSpot, vecSpot );
					UTIL_Portal_TraceRay( ray4, CONTENTS_SOLID, info.GetInflictor(), COLLISION_GROUP_NONE, &tr );
					if( tr.fraction != 1.0 )
					{
						continue;
					}
...
ConVar skill( "skill", "1", FCVAR_REPLICATED, "Game difficulty level (1-4).\n", true, 1, true, 4 );
