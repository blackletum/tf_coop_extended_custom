...
bool CScriptObject::ReadFromBuffer(const char **pBuffer, bool isNewObject)
{
	...
	*pBuffer = engine->ParseFile(*pBuffer, token, sizeof(token));
	...
	if (strcmp(token, "{"))
	{
		ConColorMsg( Color( 255, 143, 0, 255 ), "Expecting '{', got '%s'", token);
		return false;
	}
	...
	*pBuffer = engine->ParseFile(*pBuffer, token, sizeof(token));
	...
	{
		ConColorMsg( Color( 255, 143, 0, 255 ), "Type '%s' unknown", token);
		return false;
	}
...
bool CDescription::ReadFromBuffer(const char **pBuffer)
{
	...
	*pBuffer = engine->ParseFile(*pBuffer, token, sizeof(token));
	...
	{
		ConColorMsg( Color( 255, 143, 0, 255 ), "Expecting 'VERSION', got '%s'", token);
		return false;
	}
	...
	*pBuffer = engine->ParseFile(*pBuffer, token, sizeof(token));
	if (strlen(token) <= 0)
	{
		ConColorMsg( Color( 255, 143, 0, 255 ), "Expecting version #");
		return false;
	}
	...
	{
		ConColorMsg( Color( 255, 143, 0, 255 ), "Version mismatch, expecting %f, got %f", SCRIPT_VERSION, fVer);
		return false;
	}
	...
	*pBuffer = engine->ParseFile(*pBuffer, token, sizeof(token));
	...
	{
		ConColorMsg( Color( 255, 143, 0, 255 ), "Expecting 'DESCRIPTION', got '%s'", token);
		return false;
	}
	...
	*pBuffer = engine->ParseFile(*pBuffer, token, sizeof(token));
	if (strlen(token) <= 0)
	{
		ConColorMsg( Color( 255, 143, 0, 255 ), "Expecting '%s'", m_pszDescriptionType);
		return false;
	}

	if (stricmp(token, m_pszDescriptionType))
	{
		ConColorMsg( Color( 255, 143, 0, 255 ), "Expecting %s, got %s", m_pszDescriptionType, token);
		return false;
	}
	...
	while (1)
	{
		...
		*pBuffer = pStart;
		...
		else
		{
			...
			if (!pObj)
			{
				ConColorMsg( Color( 255, 143, 0, 255 ), "Couldn't create script object");
				return false;
			}
...
bool CDescription::InitFromFile(char *pszFileName)
{
	...
	FileHandle_t file = g_pFullFileSystem->Open( pszFileName, "rb", "MOD" );
	if ( !file )
		return false;
