...
void BaseModHybridButton::PaintButtonEx()
{
	...
	if ( bDrawCursor )
	{
		...
		if ( m_nStyle == BUTTON_DIALOG )
		{
			...
			vgui::surface()->DrawFilledRectFade( blotchX, y, blotchX + 0.50f * blotchWide, y+tall, 0, 150, true );
			vgui::surface()->DrawFilledRectFade( blotchX + 0.50f * blotchWide, y, blotchX + blotchWide, y+tall, 150, 0, true );
		}
		else
		{
			int blotchWide = textWide + vgui::scheme()->GetProportionalScaledValueEx( GetScheme(), 40 );
			int blotchX = x + textInsetX;
			vgui::surface()->DrawFilledRectFade( blotchX, y, blotchX + 0.25f * blotchWide, y+tall, 0, 150, true );
			vgui::surface()->DrawFilledRectFade( blotchX + 0.25f * blotchWide, y, blotchX + blotchWide, y+tall, 150, 0, true );
		}
		...
		if ( curState == Open )
		{
			...
			{
				vgui::surface()->DrawFilledRectFade( x, y, x + wide, y+2, 255, 0, true );
			}
			else
			{
				// the border lines end at the beginning of the flyout
				// the flyout will draw to complete the look
				vgui::surface()->DrawFilledRectFade( x, y, x + wide, y+2, 0, 255, true );
				vgui::surface()->DrawFilledRectFade( x, y+tall-2, x + wide, y+tall, 0, 255, true );
			}
		}
		else
		{
			// top and bottom border lines
			vgui::surface()->DrawFilledRectFade( x, y, x + 0.5f * wide, y+2, 0, 255, true );
			vgui::surface()->DrawFilledRectFade( x + 0.5f * wide, y, x + wide, y+2, 255, 0, true );
			vgui::surface()->DrawFilledRectFade( x, y+tall-2, x + 0.5f * wide, y+tall, 0, 255, true );
			vgui::surface()->DrawFilledRectFade( x + 0.5f * wide, y+tall-2, x + wide, y+tall, 255, 0, true );
		}
	}
	...
	if ( m_nStyle == BUTTON_DROPDOWN && curState != Open )
	{
		...
		int textWide, textTall;
		vgui::surface()->GetTextSize( m_hSelectionFont, pUnicodeString, textWide, textTall );
...
void BaseModHybridButton::OnCursorEntered()
{
	...
	if ( !m_isOpen )
	{
		if ( IsEnabled() && !HasFocus() )
		{
			...
			//vgui::GetAnimationController()->RunAnimationCommand( this, "Size", 2, 0.05f, 0.3f, vgui::AnimationController::INTERPOLATOR_LINEAR );
		}
...
void BaseModHybridButton::OnCursorExited()
{
	...
	//vgui::GetAnimationController()->RunAnimationCommand( this, "Size", 1, 0.05f, 0.3f, vgui::AnimationController::INTERPOLATOR_LINEAR );
	if ( IsPC() )
	{
//		SetClosed();
	}
...
Panel* BaseModHybridButton::NavigateUp()
{
	...
	if ( IsPC() && !target && (dynamic_cast< DropDownMenu * >( GetParent() ) || dynamic_cast< SliderControl * >( GetParent() )) )
...
Panel* BaseModHybridButton::NavigateLeft()
{
	...
	if ( IsPC() && !target && dynamic_cast< DropDownMenu * >( GetParent() ) )
